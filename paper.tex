...
type. It suffices to prove termination on orthodox Church-style typed
terms, because any infinite reduction on an interpretation term
induces an infinite reduction on a corresponding orthodox Church-style
typed term.
...
\tau$ if $t$ has type~$\tau$. The subject reduction lemma still holds
for orthodox Church-style typed terms.
...
\begin{lemma}\label{lem_chi_kappa_reducible}
  $\val{\chi_\kappa}{}{} \in \Cb_{\chi_\kappa}$.
\end{lemma}

\begin{proof}
  Induction on~$\kappa$. If $\kappa = *$ then this follows from
  Lemma~\ref{lem_nat_reducible}. If $\kappa=\kappa_1\arrkind\kappa_2$
...
\begin{proof}
  ...
  \item Let $t \in \val{\forall(\alpha:\kappa)[\tau]}{\xi}{\omega}$
    ...
    kind~$\kappa$. By Lemma~\ref{lem_chi_kappa_reducible} we have
  ...
  \item Let $t \in \val{\forall\alpha[\tau]}{\xi}{\omega}$. We show $s
    ...
    Lemma~\ref{lem_chi_kappa_reducible}. Since~$\chi_\kappa$ is also
