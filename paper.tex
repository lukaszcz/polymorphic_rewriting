...
\begin{definition}\normalfont
  ...
  type constructor variables, then we consider $(\alpha \tau)[\alpha
    := \lambda \beta . \beta]$ and $\tau$ and $(\lambda \beta . \beta)
  \tau$ to be the same.
\end{definition}
...
\LC{You had some mistakes in the rules below. I think these were only
  typos, but re-check if the interpretation still works.}

And finally the rules.  These are represented as \emph{rule schemes},
representing infinitely many rules at once.
\[
\begin{array}{rcl}
...
%\]
\end{itemize}
\LC{The following rules are wrong. I'll correct this on Monday.}
\begin{itemize}
\item $\epsilon_\tau(\mathtt{let}_{\varphi,\bot}(s,t)) \red
  \mathtt{let}_{\varphi,\tau}(s,\epsilon_\tau(t))$
\item $@_{\tau,\rho}(\mathtt{let}_{\varphi, \tau \arrtype
  \rho}(s,\tabs{\alpha}{\abs{x:\varphi\alpha}{t}}),u) \red
  \mathtt{let}_{\varphi,\rho}(s,@_{\tau,\rho}(t, u))$
...
\item $\mathtt{tapp}_{\quant{\alpha}{\sigma},\tau}(\tabs{\alpha}{s}) \red
  s[\alpha:=\tau]$ \\
