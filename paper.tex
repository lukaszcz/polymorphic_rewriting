...
\begin{lemma}
  ...
  \item If $t \succ t'$ (resp.~$t \succeq t'$) and $s \succeq s'$ then
    \[
    \xlet{}{t}{\alpha,x}{s} \succ \xlet{}{t}{\alpha,x}{s}
    \]
    (resp.~$\xlet{}{t}{\alpha,x}{s} \succeq \xlet{}{t}{\alpha,x}{s}$).
  \end{enumerate}
\end{lemma}
...
\Termmap(\mathtt{in}^1) & = & \Lambda \alpha \Lambda \beta \lambda x : \alpha.\quad & \pair{x}{\lift_\beta(1)} \\
\Termmap(\mathtt{in}^2) & = & \Lambda \alpha \Lambda \beta \lambda x : \beta.\quad & \pair{\lift_\alpha(1)}{x} \\
\end{array}
\]
\[
\begin{array}{rcl}
...
\Termmap(\mathtt{case}) & = & \Lambda \alpha,\beta,\xi . \lambda x : \alpha \times \beta,
  ...
  & & \quad
  \lift_\xi(37) \oplus (\lift_\xi(\flatten_{\alpha\times\beta}(x)\oplus 1) \otimes \\
...
\item Generalised, what we want is roughly the following:
  ...
  \item And: $s * (\pi \arrtype \rho) \cdot (\Lambda \vec{\beta} \lambda \vec{x}.t) \cdot u \succeq s * (\psi\tau) \cdot (\Lambda \vec{\beta} \lambda \vec{x}.t \cdot u)$
  \end{itemize}
