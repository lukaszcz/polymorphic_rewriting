...
\[
\begin{array}{rcl}
\Termmap(\epsilon_\alpha) & = & \abs{x:\nat}{\mathtt{lift}_\alpha(x)} \\
\Termmap(@_{\alpha,\beta}) & = & \abs{x: \alpha \arrtype \beta,y :
  \alpha}{\oplus_{\beta} \cdot (x \cdot y) \cdot \mathtt{lift}_\beta(1)}
  \\
\Termmap(\mathtt{ext}_{\alpha,\beta}) & = & \abs{x:\alpha\langle\beta
  \rangle}{\tabs{\xi}{\abs{y:\quant{\chi}{\alpha\langle\chi\rangle
