...
\Termmap(\mathtt{let}) & = & \Lambda \alpha : * \arrkind * . \Lambda \beta : * . \lambda x : \Sigma \xi . \alpha\xi,
  ...
  & & \hfill (\xlet{\beta}{x}{\xi,z}{y\xi z}) \oplus y \cdot \lift_{\alpha
  \xi}(0) \oplus \lift_\beta(1) \\
...
\item[removing non-zero multiplicants]
  $\lift_\xi(t \oplus n) \otimes s \succeq s$ if $n > 0$
...
\item $\interpret{@_{\sigma,\tau}(\abs{x}{s},t)} \succ
  ...
  \interpret{t} \oplus
  \lift_\tau(\flatten_\sigma(\interpret{t}) \oplus 1) \leadsto
  \interpret{s}[x:=\interpret{t}] \oplus \lift(\flatten(\interpret{t})
  \oplus 1) \succ \interpret{s}[x:=\interpret{t}]$ (by absolute
...
\item $\interpret{\pi^1_{\sigma,\tau}(\mathtt{pair}_{\sigma,\tau}(s,t))}
  ...
  \pair{\interpret{s}}{\interpret{t}} \oplus \lift_{\sigma,\tau}(
  \flatten_{\sigma}(\interpret{s}) \oplus \flatten_{\tau}(\interpret{
  t}))$ and therefore
  $\interpret{\pi^1_{\alpha,\beta}(\mathtt{pair}_{\alpha,\beta}(s,t))}
  \arrrbeta \pi^1(\pair{\interpret{s}}{\interpret{t}} \oplus \langle
  \text{something}\rangle) \oplus \lift_\sigma(1) \succeq
  \pi^1(\pair{\interpret{s}}{\interpret{t}}) \oplus \lift_\sigma(1)$ by
...
\item $\interpret{\mathtt{case}_{\sigma,\tau,\rho}(\mathtt{in}^1_{
  ...
  Write $A := \lift_{\sigma \times \tau}(\flatten_\sigma(\interpret{u}))$;
  then $\interpret{\mathtt{in}^1_{\sigma,\tau}(u)} =
  \pair{\interpret{u}}{\lift_\tau(1)} \oplus A$.
  Additionally, let $B := \flatten_{\sigma \times \tau}(
  \pair{\interpret{u}}{\lift_\tau(1)} \oplus A)$ and
  $C := \interpret{\abs{y}{t}} \cdot \pi^2(
  \pair{\interpret{u}}{\lift_\tau(1)} \oplus A)$.
  Then we can write:
  $\interpret{\mathtt{case}_{\sigma,\tau,\rho}(\mathtt{in}^1_{
  \sigma,\tau}(u),\abs{x}{s},\abs{y}{t})} = \lift_\rho(2) \oplus
  \lift_\rho(3 \otimes B) \oplus
  \lift_\rho(B \oplus 1) \otimes (\ 
  \interpret{\abs{x}{s}} \cdot \pi^1(\pair{\interpret{u}}{
  \lift_\tau(1)} \oplus A) \oplus C\ )$.
...
\item $\interpret{\mathtt{let}_{\varphi,\rho}(
  ...
  \expair{\tau,\interpret{s}}$ by absolute positiveness.
  Therefore, using monotonicity,
  $\interpret{\mathtt{let}_{\varphi,\rho}(\mathtt{ext}_{\varphi,\tau}(
  s),\tabs{\alpha}{\abs{x:\varphi \alpha}{t}})} \succeq
  (\xlet{\rho}{
    \expair{\tau}{\interpret{s}}
  }{
    \expair{\alpha}{x}
  }{\\
  ...
  \interpret{\tabs{\alpha}{\abs{x:\varphi \alpha}{t}}} \cdot
    \lift_{\varphi\alpha}(0) \oplus
  \lift_\rho(1)$.
  Again by absolute positiveness, this $\succ
  \xlet{\rho}{\expair{\tau}{\interpret{s}}}{\expair{\alpha}{x}}{
  \interpret{\tabs{\alpha}{\abs{x:\varphi \alpha}{t}}} * \alpha \cdot x
  } \leadsto
  \xlet{\rho}{\expair{\tau}{\interpret{s}}}{\expair{\alpha}{x}}{
  \interpret{t}}$.
  By Lemma \ref{lem:extreduce}, this term
  $\succeq \interpret{t}[\alpha:=\tau][x:=\interpret{s}]$.
...
\item $\interpret{\epsilon_\tau(\epsilon_\bot(s))} \succ
  ...
  \lift_{\typeinterpret{\tau}}(\lift_\nat(\interpret{s} \oplus 1)
  \oplus 1) \approx \lift_{\typeinterpret{\tau}}(\interpret{s} \oplus
  2) \succ \lift_{\typeinterpret{\tau}}(\interpret{s} \oplus 1) =
  \interpret{\epsilon_\tau(s)}$.
\item $\interpret{@_{\sigma,\tau}(\epsilon_{\sigma \arrtype \tau}(s),
  ...
  \interpret{s} \oplus 1) \cdot \interpret{t} \oplus
  \lift_{\typeinterpret{\tau}}(\flatten_{\typeinterpret{\sigma}}(
  \interpret{t}) \oplus 1) \succeq
  \lift_{\typeinterpret{\sigma} \arrtype \typeinterpret{\tau}}(
  \interpret{s} \oplus 1) \cdot \interpret{t} \leadsto
  \typeinterpret{\tau}(\interpret{s} \oplus 1) = \interpret{
  \epsilon_\tau(s)}$.
\item $\interpret{\mathtt{tapp}_{\varphi,\tau}(
  ...
  (\tabs{\alpha}{\lift_{\typeinterpret{\varphi}\alpha}(
  \interpret{s} \oplus 1)}) * \typeinterpret{\tau} \leadsto
  \lift_{\typeinterpret{\varphi}\typeinterpret{\tau}}(
  \interpret{s} \oplus 1) =
  \lift_{\typeinterpret{\varphi\tau}}(\interpret{s} \oplus 1) =
  \interpret{\epsilon_{\varphi\tau}(s)}$
\item $\interpret{\pi^1_{\sigma,\tau}(\epsilon_{\mathtt{and}\,
  ...
  \sigma\,\tau}(s))} = \pi^1(\lift_{\typeinterpret{\sigma} \times
  \typeinterpret{\tau}}(\interpret{s} \oplus 1)) \oplus
  \lift_{\typeinterpret{\sigma}}(1) \succ\linebreak
  \pi^1(\lift_{\typeinterpret{\sigma} \times
  \typeinterpret{\tau}}(\interpret{s} \oplus 1)) =
  \lift_{\forall p.(\typeinterpret{\sigma} \arrtype
  \typeinterpret{\tau} \arrtype p) \arrtype p}(\interpret{s} \oplus 1))
  * \typeinterpret{\sigma} \cdot (\lambda xy.x) =
  (\tabs{p}{\lambda f.\lift_p(\interpret{s} \oplus 1)}) *
  \typeinterpret{\sigma} \cdot (\lambda xy.x) \leadsto^*
  \lift_{\typeinterpret{\sigma}}(\interpret{s} \oplus 1) =
...
\item $\interpret{\mathtt{case}_{\sigma,\tau,\rho}(
  ...
  \lift_\rho(3 \otimes \flatten_{\interpret{\sigma} \times
    \interpret{\tau}}(\lift_{\interpret{\sigma} \times
    \interpret{\tau}}(\interpret{u} \oplus 1))) \oplus
    \langle\text{something}\rangle \succ
  \lift_\rho(3 \otimes \flatten_{\interpret{\sigma} \times
    \interpret{\tau}}(\lift_{\interpret{\sigma} \times
    \interpret{\tau}}(\interpret{u} \oplus 1))) \succeq\\
  \lift_\rho(\flatten_{\interpret{\sigma} \times
    \interpret{\tau}}(\lift_{\interpret{\sigma} \times
    \interpret{\tau}}(\interpret{u} \oplus 1))) \approx
  \lift_\rho(\interpret{u} \oplus 1) =
...
\item $\interpret{\mathtt{let}_{\varphi,\rho}(
  ...
  $\interpret{\mathtt{let}_{\varphi,\rho}(\epsilon_{
    ...
    \varphi}\alpha}(\interpret{s} \oplus 1)}{\expair{\alpha}{x}}{
    (\tabs{\alpha}{\abs{x}{\interpret{t}}}) * \alpha \cdot x}) \oplus
    \langle\text{something}\rangle \oplus
    \lift_{\typeinterpret{\rho}}(1) \succ \cdot \leadsto^*
    \xlet{\typeinterpret{\rho}}{\lift_{\Sigma \alpha.\typeinterpret{
    \varphi}\alpha}(\interpret{s} \oplus 1)}{\expair{\alpha}{x}}{
    \interpret{t}} =
    \lift_{\forall p.(\forall \alpha.
    \typeinterpret{\varphi}\alpha \arrtype p) \arrtype p}(\interpret{s}
    \oplus 1) * \typeinterpret{\rho} \cdot (\tabs{\alpha}{\abs{x}{
    \interpret{t}}}) \leadsto^*
    \lift_{\typeinterpret{\rho}}(\interpret{s} \oplus 1) =
...
\Termmap(\mathtt{case}) & = & \Lambda \alpha,\beta,\xi . \lambda x : \alpha \times \beta,
  ...
  & & \quad \lift_\xi(5) \oplus \\
  & & \quad \lift_\xi(11 \otimes \flatten_{\alpha \times
    \beta}(x)) \oplus y \cdot \pi^1(x) \oplus z \cdot \pi^2(x) \oplus\\
  & & \quad
    \lift_\xi(3 \otimes \flatten_{\alpha \times \beta}(x)) \otimes
    y \cdot \pi^1(x) \oplus \\
  & & \quad
    \lift_\xi(3 \otimes \flatten_{\alpha \times \beta}(x)) \otimes
    z \cdot \pi^2(x)
...
\item Let's fill in the inner $\mathtt{case}$ interpretations in the
  ...
  \mathcal{J}(\mathtt{case})_{\sigma,\tau,\xi}(\interpret{u},\abs{x}{
    \lift_\xi(5) \oplus
      \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(\interpret{s}))
      \oplus \\
    \phantom{ABCDEFGHIJKL}
      (\lambda z.\interpret{v}) \cdot \pi^1(\interpret{s}) \oplus
      (\lambda a.\interpret{w}) \cdot \pi^2(\interpret{s}) \oplus \\
    \phantom{ABCDEFGHIJKL}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(\interpret{s}))
      \otimes (\lambda z.\interpret{v}) \cdot \pi^1(\interpret{s}) \oplus\\
    \phantom{ABCDEFGHIJKL}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(\interpret{s}))
      \otimes (\lambda a.\interpret{w}) \cdot \pi^2(\interpret{s})
    },\ \\
    \phantom{ABCDEFGHIJ}\abs{y}{
    \lift_\xi(5) \oplus
      \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(\interpret{t}))
      \oplus \\
    \phantom{ABCDEFGHIJKL}
      (\lambda z.\interpret{v}) \cdot \pi^1(\interpret{t}) \oplus
      (\lambda a.\interpret{w}) \cdot \pi^2(\interpret{t}) \oplus \\
    \phantom{ABCDEFGHIJKL}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(\interpret{t}))
      \otimes (\lambda z.\interpret{v}) \cdot \pi^1(\interpret{t}) \oplus\\
    \phantom{ABCDEFGHIJKL}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(\interpret{t}))
      \otimes (\lambda a.\interpret{w}) \cdot \pi^2(\interpret{t})
    }\ )
$
\item And $\beta$-normalise them.  We're allowed to do that by
  ...
  \mathcal{J}(\mathtt{case})_{\sigma,\tau,\xi}(\interpret{u},\abs{x}{
    ...
    \phantom{ABCDEFGHIJKL}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(\interpret{s}))
      \otimes \interpret{v}[z:=\pi^1(\interpret{s})] \oplus \\
    \phantom{ABCDEFGHIJKL}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(\interpret{s}))
    ...
    \phantom{ABCDEFGHIJKL}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(\interpret{t}))
      \otimes \interpret{v}[z:=\pi^1(\interpret{t})] \oplus\\
    \phantom{ABCDEFGHIJKL}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(\interpret{t}))
...
\item Now we do the inner case interpretation in the left-hand side; we
  ...
  $\mathcal{J}(\mathtt{case})_{\rho,\pi,\xi}( \\
  \phantom{ABC}
    \lift_{\rho \times \pi}(5) \oplus
    \lift_{\rho \times \pi}(11 \otimes \flatten_{\sigma \times
      \tau}(\interpret{u})) \oplus \\
  \phantom{ABC}\interpret{s}[x:=\pi^1(\interpret{u})]
    \oplus \interpret{t}[y:=\pi^2(\interpret{u})] \oplus \\
  \phantom{ABC}
    \lift_{\rho \times \pi}(3 \otimes \flatten_{\sigma \times \tau}(
  ...
  \phantom{A}
  , \abs{z}{\interpret{v}},,\ \abs{a}{\interpret{w}}\ )\ \succ \\
  \mathcal{J}(\mathtt{case})_{\sigma,\tau,\xi}(\interpret{u},\abs{x}{
    \lift_\xi(5) \oplus
      \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(\interpret{s}))
      \oplus \\
...
$
\item Once I start filling in the outer $\mathtt{case}$ interpretations
  on both sides, this is going to get very messy indeed.  So, I will
  use the following shorthand notations: \\
  $A = \\
  \phantom{ABC}\lift_{\rho \times \pi}(5) \oplus
    \lift_{\rho \times \pi}(11 \otimes \flatten_{\sigma \times
      \tau}(\interpret{u})) \oplus \\
  \phantom{ABC}\interpret{s}[x:=\pi^1(\interpret{u})]
    \oplus \interpret{t}[y:=\pi^2(\interpret{u})] \oplus \\
  ...
  \phantom{ABC}
    \lift_{\rho \times \pi}(3 \otimes \flatten_{\sigma \times \tau}(
      \interpret{u})) \otimes \interpret{t}[y:=\pi^2(\interpret{u})]
  $ \\
  and for a given term $q$: \\
  $B_q =
    \lift_\xi(5) \oplus
      \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(q)) \oplus \\
    \phantom{ABCDEFGHIJKL}
      \interpret{v}[z:=\pi^1(q)] \oplus
      \interpret{w}[a:=\pi^2(q)] \oplus \\
    \phantom{ABCDEFGHIJKL}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(q))
      \otimes \interpret{v}[z:=\pi^1(q)] \oplus \\
    \phantom{ABCDEFGHIJKL}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(q))
      \otimes \interpret{w}[a:=\pi^2(q)]
  $. \\
  Thus we can rewrite the inequality to: \\
  $\mathcal{J}(\mathtt{case})_{\rho,\pi,\xi}(\ A,
    \abs{z}{\interpret{v}},,\ \abs{a}{\interpret{w}}\ )\ \succ \\
  \mathcal{J}(\mathtt{case})_{\sigma,\tau,\xi}(\ \interpret{u},\abs{x}{
    B_{\interpret{s}}}, B_{\interpret{t}} )
$
\item Now I will fill in the outer $\mathtt{case}$ interpretations
  on both sides, and immediately $\beta$-reduce them. \\
  $\lift_\xi(5) \oplus
    \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(A)) \oplus \\
    \phantom{AB}
    \interpret{v}[z:=\pi^1(A)] \oplus
    \interpret{w}[a:=\pi^2(A)] \oplus \\
    \phantom{AB}
    \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(A)) \otimes
      \interpret{v}[z:=\pi^1(A)] \oplus \\
    \phantom{AB}
    \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(A)) \otimes
  ...
  \succ \\
    \lift_\xi(5) \oplus
    \lift_\xi(11 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
    \oplus \\
    \phantom{AB}
    B_{\interpret{s}}[x:=\pi^1(\interpret{u})] \oplus
    B_{\interpret{t}}[x:=\pi^2(\interpret{u})] \oplus \\
    \phantom{AB}
    \lift_\xi(3 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
      \otimes B_{\interpret{s}}[x:=\pi^1(\interpret{u})] \oplus \\
    \phantom{AB}
    \lift_\xi(3 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
      \otimes B_{\interpret{t}}[x:=\pi^2(\interpret{u})]
  $
\item Note that $s$ is a bound variable in $s$ and $y$ a bound variable
  ...
  \succ \\
    ...
    \lift_\xi(3 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
      \otimes B_{\interpret{t}[x:=\pi^2(\interpret{u})]}
  $
\item Now, let us expand the occurrences of $B_q$ on the right; we
  will denote $su$ for $\interpret{s}[x:=\pi^1(\interpret{u})]$ and
  $tu$ for $\interpret{t}[x:=\pi^2(\interpret{u})]$.  We use the
  distribution property \CK{(TODO)} to obtain the following requirement:
  \\
  $\lift_\xi(5) \oplus
    \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(A)) \oplus \\
    \phantom{AB}
  ...
  \succ \\
    ...
    \phantom{AB}
      \lift_\xi(5) \oplus \\
    \phantom{AB}
      \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(su)) \oplus \\
    \phantom{AB}
      \interpret{v}[z:=\pi^1(su)] \oplus \\
    \phantom{AB}
      \interpret{w}[a:=\pi^2(su)] \oplus \\
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(su))
      \otimes \interpret{v}[z:=\pi^1(su)] \oplus \\
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(su))
      \otimes \interpret{w}[a:=\pi^2(su)] \oplus \\
    \\
    \phantom{AB}
      \lift_\xi(5) \oplus \\
    \phantom{AB}
      \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(tu)) \oplus \\
    \phantom{AB}
      \interpret{v}[z:=\pi^1(tu)] \oplus \\
    \phantom{AB}
      \interpret{w}[a:=\pi^2(tu)] \oplus \\
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(tu))
      \otimes \interpret{v}[z:=\pi^1(tu)] \oplus \\
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(tu))
      \otimes \interpret{w}[a:=\pi^2(tu)] \oplus \\
  \\
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
        \otimes \lift_\xi(5) \oplus \\
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
      \otimes
      \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(su)) \oplus \\
    ...
    \phantom{AB}
      ...
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(su))
      \otimes \\
      \phantom{ABCD}\interpret{v}[z:=\pi^1(su)] \oplus \\
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
      \otimes
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(su))
      \otimes \\
      \phantom{ABCD}\interpret{w}[a:=\pi^2(su)] \oplus \\
    \\
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
      \otimes
    ...
    \phantom{AB}
      ...
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(tu))
      \otimes \\
      \phantom{ABCD}\interpret{v}[z:=\pi^1(tu)] \oplus \\
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
      \otimes
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(tu))
      \otimes \\
      \phantom{ABCD}\interpret{w}[a:=\pi^2(tu)] \\
  $
\item Yes, that's a monstrosity, and it will only get worse once we
  get to the left-hand side. Let's combine what we can, using the
  properties that for $R \in \{\oplus,\otimes\}$ always
  $a R b \approx b R a$ and $\lift_\xi(a) R \lift_\xi(b) \approx
  \lift_\xi(a R b)$, and also that $a \approx \lift(1) \otimes a$.
  Let us also denote:
  \begin{itemize}
  ...
  \end{itemize}
  Additionally striking away the (first) occurrences of $\lift_\xi(5)$
  on both the left and the right against each other, this gives: \\
  $\lift_\xi(11 \otimes \flatten_{\rho \times \pi}(A)) \oplus \\
    \phantom{AB}
    \interpret{v}[z:=\pi^1(A)] \oplus
    \interpret{w}[a:=\pi^2(A)] \oplus \\
  ...
  \succ \\
    \lift_\xi(5 + 5) \oplus \\
    \phantom{AB}
    \lift_\xi((11 \oplus 3 \otimes 5 \oplus 3 \otimes 5) \otimes
      \flatten_{\sigma \times \tau}(\interpret{u})) \oplus \\
    \\
    \phantom{AB}
      \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(su)) \oplus \\
    \phantom{AB}
      \lift_\xi(1 \oplus 3 \otimes \flatten_{\rho \times \pi}(su))
      \otimes vsu \oplus \\
    \phantom{AB}
      \lift_\xi(1 \oplus 3 \otimes \flatten_{\rho \times \pi}(su))
      \otimes wsu \oplus \\
    ...
    \phantom{AB}
      \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(tu)) \oplus \\
    \phantom{AB}
      \lift_\xi(1 \oplus 3 \otimes \flatten_{\rho \times \pi}(tu))
      \otimes vtu \oplus \\
    \phantom{AB}
      \lift_\xi(1 \oplus 3 \otimes \flatten_{\rho \times \pi}(tu))
      \otimes wtu \oplus \\
    \\
    \phantom{AB}
      \lift_\xi(3 \otimes 11 \otimes
        \flatten_{\sigma \times \tau}(\interpret{u}) \otimes
        \flatten_{\rho \times \pi}(su)) \oplus \\
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
      \otimes vsu \oplus \\
    \phantom{AB}
      ...
      \otimes wsu \oplus \\
    \phantom{AB}
      \lift_\xi((3 \otimes 3) \otimes \flatten_{\sigma \times
        \tau}(\interpret{u}) \otimes \flatten_{\rho \times \pi}(su))
      \otimes vsu \oplus \\
    \phantom{AB}
      \lift_\xi((3 \otimes 3) \otimes \flatten_{\sigma \times
        \tau}(\interpret{u}) \otimes \flatten_{\rho \times \pi}(su))
    ...
    \\
    \phantom{AB}
      \lift_\xi(3 \otimes 11 \otimes \flatten_{\sigma \times
      \tau}(\interpret{u}) \otimes \flatten_{\rho \times \pi}(tu))
      \oplus \\
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
      \otimes vtu \oplus \\
    ...
    \phantom{AB}
      ...
      \otimes wtu \\
  $
\item This is starting to get just slightly unmanageable, so let's not
  try to expand the left-hand side all in one go.  Rather, let's look
  at what happens to the very first term.  Note that \\
  $A = \\
  \phantom{ABC}\lift_{\rho \times \pi}(5) \oplus
    \lift_{\rho \times \pi}(11 \otimes \flatten_{\sigma \times
      \tau}(\interpret{u})) \oplus su \oplus tu \oplus \\
  \phantom{ABC}
    \lift_{\rho \times \pi}(3 \otimes \flatten_{\sigma \times \tau}(
  ...
  $ \\
  So, using that always $\flatten(\lift(n)) = n$ and that
  $\flatten(a \oplus b) = \flatten(a) + \flatten(b)$ and that
  $\lift(a + b) = \lift(a) \oplus \lift(b)$ \CK{(TODO)} we can
  \emph{partially} expand the first part of the left-hand side as
  follows: \\
  $\lift_\xi(11 \otimes \flatten_{\rho \times \pi}(A)) = \\
  \phantom{AB}
  \lift_\xi(11 \otimes 5) \oplus \\
  \phantom{AB}
  \lift_\xi(11 \otimes 11 \otimes \flatten_{\sigma \times \tau}(
    \interpret{u})) \oplus \\
  \phantom{AB}
  \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(su)) \oplus \\
  \phantom{AB}
  \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(tu)) \oplus \\
  \phantom{AB}
  \lift_\xi(11 \otimes 3 \otimes
    \flatten_{\sigma \times \tau}(\interpret{u}) \otimes
    \flatten_{\rho \times \pi}(su)) \oplus \\
  \phantom{AB}
  \lift_\xi(11 \otimes 3 \otimes \flatten_{\sigma \times \tau}(
    \interpret{u}) \otimes \flatten_{\rho \times \pi}(tu))
  $
\item This leaves the following inequality: \\
  $\lift_\xi(11 \otimes 5) \oplus \hfill (L1) \\
  \phantom{AB}
  \lift_\xi(11 \otimes 11 \otimes \flatten_{\sigma \times \tau}(
    \interpret{u})) \oplus \hfill (L2) \\
  \phantom{AB}
  \lift_\xi(11 \otimes \flatten_{\rho \times \pi}(su)) \oplus
    \hfill (L3) \\
  \phantom{AB}
  ...
  \phantom{AB}
  \lift_\xi(11 \otimes 3 \otimes \flatten_{\sigma \times \tau}(
    \interpret{u}) \otimes \flatten_{\rho \times \pi}(tu)) \oplus
    \hfill (L6) \\
  \phantom{AB}
    \interpret{v}[z:=\pi^1(A)] \oplus
    \interpret{w}[a:=\pi^2(A)] \oplus \\
  ...
  \succ \\
    ...
    \phantom{AB}
      ...
      \otimes wtu
  $
\item Note that we can strike each of (L3)/(R3)--(L6)/(R6) away
  against each other.  Moreover, since
  $11 \otimes 5 > 5 + 5$ % CONSTANTS!
  we have $(L1) \succ (R1)$.  We also have $(L2) \succeq (R2)$.
  By absolute positiveness \CK{(TODO)}, we thus obtain: \\
  $\interpret{v}[z:=\pi^1(A)] \oplus
    \interpret{w}[a:=\pi^2(A)] \oplus \\
  \phantom{AB}
    \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(A)) \otimes
  ...
  \succeq \\
    ...
    \phantom{AB}
      ...
      \otimes wtu
  $
\item Now, we can use the same technique for dealing with the later
  uses of $\flatten_{\rho \times \pi}(A)$.  Note that for any
  $q$, we have: \\
  $\lift_\xi(3 \otimes \flatten_{\rho \times \pi}(A)) \otimes q = \\
  \phantom{AB}
  \lift_\xi(3 \otimes 5) \otimes q \oplus \\
  \phantom{AB}
  \lift_\xi(3 \otimes 11 \otimes \flatten_{\sigma \times \tau}(
    \interpret{u})) \otimes q \oplus \\
  \phantom{AB}
  \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(su)) \otimes q \oplus \\
  \phantom{AB}
  \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(tu)) \otimes q \oplus \\
  \phantom{AB}
  \lift_\xi(3 \otimes 3 \otimes
    \flatten_{\sigma \times \tau}(\interpret{u}) \otimes
    \flatten_{\rho \times \pi}(su)) \otimes q \oplus \\
  \phantom{AB}
  \lift_\xi(3 \otimes 3 \otimes \flatten_{\sigma \times \tau}(
    \interpret{u}) \otimes \flatten_{\rho \times \pi}(tu)) \otimes q
  $. \\
  Note also that both $A \succeq su$ and $A \succeq tu$ (since both
  are parts).  Therefore, and by monotonicity of $\succeq$, we have
  $\interpret{v}[z:=\pi^1(A)] \succeq \interpret{v}[z:=su] = vsu$ and
  $\interpret{v}[z:=\pi^1(A)] \succeq \interpret{v}[z:=tu] = vtu$.
  Consequently, \\
  $\lift_\xi(3 \otimes \flatten_{\rho \times \pi}(A)) \otimes
      \interpret{v}[z:=\pi^1(A)] \succeq \\
  \phantom{AB}
  vsu \oplus vtu \oplus \hfill \text{(since $3 \otimes 5 \geq 1 + 1$)} \\
    % CONSTANTS!
  \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\sigma \times \tau}(\interpret{u}))
      \otimes vsu \oplus
      ...
      \otimes vtu \\
      \phantom{ABCD}
      \oplus \hfill \text{(since $3 \otimes 11 > 3 + 3$)} \\
      % CONSTANTS!
    \phantom{AB}
      \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(su))
      \otimes vsu \oplus \\
    ...
    \phantom{AB}
      \lift_\xi(3 \otimes 3 \otimes
      \flatten_{\sigma \times \tau}(\interpret{u}) \otimes
      \flatten_{\rho \times \pi}(su)) \otimes vsu \oplus \\
    \phantom{AB}
      \lift_\xi(3 \otimes 3 \otimes \flatten_{\sigma \times \tau}(
      \interpret{u}) \otimes \flatten_{\rho \times \pi}(tu)) \otimes vtu
  $.
\item Removing these parts on both the left and the right of the
  inequality leaves the following:
  $\interpret{v}[z:=\pi^1(A)] \oplus
    \interpret{w}[a:=\pi^2(A)] \oplus \\
  \phantom{AB}
    \lift_\xi(3 \otimes \flatten_{\rho \times \pi}(A)) \otimes
      \interpret{w}[a:=\pi^1(A)] \\
\succeq \\
    \phantom{AB}
      \lift_\xi(1 \oplus 3 \otimes \flatten_{\rho \times \pi}(su))
      \otimes wsu \oplus
    ...
    \phantom{AB}
      ...
      \otimes wtu \hfill (R6)
  $ \\
  These last items on the right are easily removed by expanding the
  remaining $\flatten_{\rho \times \pi}(A)$.
\item The proof of case/case is now complete.  Note that, given the
  requirements on the constants that we encountered, it would have
  been sufficient to take:
  \begin{itemize}
  \item $3$ instead of $11$
  \item $2$ instead of $5$
  \item $1$ instead of $3$
  \end{itemize}
  Doing this would make the interpretation seem somewhat simpler
  (although honestly not that much).
\end{itemize}
