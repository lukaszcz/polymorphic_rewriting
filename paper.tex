...
well as the infinite set $\Typevars$ of type variables.
$\TypeConstructors$ must satisfy the property that if it contains a
symbol $\mathtt{f}_{\sigma_1,\dots,\sigma_n}$, then it also contains
$\mathtt{f}_{\sigma_1\gamma,\dots,\sigma_n\gamma}$ for every type
substitution $\gamma$.  The set $\Types$ of types is given by:
\begin{itemize}
...
paired with an arity $k$ and a type $\sigma_1 \arrtype \dots \arrtype
\sigma_k \arrtype \tau$, as well as the infinite set $\Vars$ of variables.
...
\begin{itemize}
\item $\Gamma \vdash x : \sigma$ for every $(x : \sigma) \in \Gamma$.
\item $\Gamma \vdash \mathtt{f}(s_1,\dots,s_k) : \tau$ if
  $(\mathtt{f} : (k,\sigma_1 \arrtype \dots \arrtype \sigma_k \arrtype
...
of application can be modelled simply using a set of symbols $\{
@_{\sigma,\tau}: (2,(\sigma \arrtype \tau) \arrtype \sigma \arrtype \tau)
\mid \sigma,\tau \in \Types \} \subseteq \Sigma$.  Similarly, there is
no type application; an application $\tapp{s}{\tau}$ with $s : \sigma$
could be modelled through a symbol $\mathtt{A}_{\sigma,\tau}(s)$.
...
\CK{Next: descritve what rules look like.  Q:
...
TODO: maybe describe system F too?
