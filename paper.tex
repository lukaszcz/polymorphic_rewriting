...
By~$\SN$ we denote the set of all terminating interpretation terms. We
use the notation $t u$ for either $\app{t}{u}$ when~$u$ is a term, or
$\tapp{t}{u}$ when~$u$ is a type.
...
\begin{definition}\label{def_candidate}
  ...
  \begin{itemize}
  \item[4.] if $t$ is neutral and in normal form then $t \in X$.
  \end{itemize}
...
\begin{proof}
  ...
  \item Let $t$ be neutral and $\Gamma \proves t : \omega(\tau_1 \to \tau_2)$
    and assume for every~$t'$ with $t \leadsto t'$ we have
    ...
    $u \in \SN$. By induction on the length of the longest reduction
    starting at~$u$ we show that
...
\begin{proof}
  We have
  $\Gamma \proves \circ_{\omega(\sigma)} : \omega(\sigma \to \sigma
  \to \sigma)$ by Definition~\ref{def_typing}. Let
  $u_1,u_2 \in \val{\sigma}{\omega,\xi}{\Gamma}$. By
  Lemma~\ref{lem_val_computable} we have $u_1,u_2 \in \SN$. Hence
  $\circ_{\omega(\sigma)} u_1 u_2 \in \SN$ follows from definitions
  (TODO: this is not completely trivial!). By induction on the length
  of the longest reduction from $\circ_{\omega(\sigma)} u_1 u_2$ to
  normal form we show that
  $\circ_{\omega(\sigma)} u_1 u_2 \in
  \val{\sigma}{\omega,\xi}{\Gamma}$, for
  arbitrary~$\sigma,\omega,\xi$. The term
  $\circ_{\omega(\sigma)} u_1 u_2$ is neutral, so it suffices to show
  that for every~$t$ with $\circ_{\omega(\sigma)} u_1 u_2 \leadsto t$
  we have $t \in \val{\sigma}{\omega,\xi}{\Gamma}$. If
  $t = \circ_{\omega(\sigma)} u_1' u_2$ or
  $t = \circ_{\omega(\sigma)} u_1 u_2'$ with $u_i \leadsto u_i'$ then
  this follows from the inductive hypothesis. Assume
  $\omega(\sigma) = \nat$ and $u_1,u_2 \in \Nbb$ and
  $\circ_{\omega(\sigma)} u_1 u_2 \leadsto (n_1 \circ n_2) = t$. If
  $\sigma = \nat$ then
  $t \in \val{\nat}{\omega,\xi}{\Gamma} =
  \val{\sigma}{\omega,\xi}{\Gamma}$. Otherwise $\sigma = \alpha$ is a
  type variable and $\omega(\alpha) = \nat$. Then
  $t \in \xi(\alpha) = \val{\sigma}{\omega,\xi}{\Gamma}$ because~$t$
  is neutral and normal. Assume $\omega(\sigma) = \tau_1 \to \tau_2$
  and $u_1 = \lambda x : \tau_1 . u_1'$,
  $u_2 = \lambda x : \tau_1 . u_2'$ and
  $t = \lambda x : \tau_1 . \circ_{\tau_2} u_1' u_2'$. Let
  $u \in \val{\tau_1}{}{\Gamma}$ TODO
\end{proof}
...
\begin{proof}
  By induction on~$t$, using the previous lemmas.
\end{proof}
