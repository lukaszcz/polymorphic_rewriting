...
\begin{definition}\label{def_wm_candidate}\normalfont
  ...
  terms of type~$\tau$ is a \emph{candidate of type~$\tau$} if it
  ...
  A binary relation~$\ge$ on a candidate $X \in \Cb_\tau$ is a
  \emph{comparison candidate for~$X$} if it satisfies the following
  properties:
  \begin{enumerate}
  \item ${\succeq} \cap X \times X \subseteq {\ge}$,
  \item if $t_1 \ge t_2$ and $t_1' \leadsto t_1$ (resp.~$t_2' \leadsto
    t_2$) then $t_1' \ge t_2$ (resp.~$t_1 \ge t_2'$),
  \item if $t_1 \ge t_1'$ and $t_2 \ge t_2'$ then $\circ_\tau t_1 t_2
    \ge \circ_\tau t_1' t_2'$ for $\circ \in \{\oplus,\otimes\}$,
  \item if $n \ge m$ (in natural numbers) then $\lift_\tau(n) \ge
    \lift_\tau(m)$.
  \end{enumerate}
...
\begin{definition}\label{def_wm_valuation}\normalfont
  ...
  variables such that if $\xi(\alpha) = (X,\ge)$ then $X \in
  \Cb_{\omega(\alpha)}$ and $\ge$ is a comparison candidate for~$X$.

  For each type constructor~$\sigma$, each mapping~$\omega$ closed
  for~$\sigma$, and each $\omega$-valuation~$\xi$, the set
  $\val{\sigma}{\xi}{\omega}$, the
  relation~$\gteq{\sigma}{\xi}{\omega}$ for~$\sigma$ a type, and the
  function $\gteq{\sigma}{\xi}{\omega}$ for~$\sigma$ not a type, are
  defined by mutual induction on~$\sigma$:
  \begin{itemize}
  \item $\val{\alpha}{\xi}{\omega} = X$ if $\xi(\alpha) = (X, \ge)$
    for a type constructor variable~$\alpha$,
  ...
  \item $\val{\sigma \arrtype \tau}{\xi}{\omega}$ is the set of all
    ...
    \item if $s_1 \gteq{\sigma}{\xi}{\omega} s_2$ and $s_i \in
      \val{\sigma}{\xi}{\omega}$ then $\app{t}{s_1}
      \gteq{\tau}{\xi}{\omega} \app{t}{s_2}$,
    \end{itemize}
  \item $\val{\forall(\alpha:\kappa)[\sigma]}{\xi}{\omega}$ is the set
    of all terms~$t$ such that $t : \omega(\forall\alpha[\sigma])$ and
    for every closed type constructor~$\varphi$ of kind~$\kappa$ and
    every $X \in \Cb_\varphi$ and every comparison candidate~$\ge$
    for~$X$ we have $\tapp{t}{\varphi} \in
    \val{\sigma}{\xi[\subst{\alpha}{(X,\ge)}]}{\omega[\subst{\alpha}{\varphi}]}$,
  \item
    $\val{\varphi \psi}{\xi}{\omega} =
    \val{\varphi}{\xi}{\omega}(\omega(\psi),\val{\psi}{\xi}{\omega})$,
  \item
    ...
    \val{\varphi}{\xi[\subst{\alpha}{X}]}{\omega[\subst{\alpha}{\psi}]}$
    for closed $\psi \in \Tc_\kappa$ and $X \in \Cb_\psi$,
  \item ${\gteq{\alpha}{\xi}{\omega}} = {\ge}$ if $\xi(\alpha) = (X, \ge)$,
  \item ${\gteq{\nat}{\xi}{\omega}} = {\succeq_\nat}$,
  \item $t_1 \gteq{\sigma\arrtype\tau}{\xi}{\omega} t_2$ iff $t_1,t_2
    \in \val{\sigma\arrtype\tau}{\xi}{\omega}$ and for every $s \in
    \val{\sigma}{\xi}{\omega}$ we have $t_1 s \gteq{\tau}{\xi}{\omega}
    t_2 s$,
  \item $t_1 \gteq{\forall(\alpha:\kappa)[\sigma]}{\xi}{\omega} t_2$
    iff $t_1,t_2 \in
    \val{\forall(\alpha:\kappa)[\sigma]}{\xi}{\omega}$ and for every
    closed type constructor~$\varphi$ of kind~$\kappa$, every $X \in
    \Cb_\varphi$ and every comparison candidate~$\ge$ for~$X$ we have
    $t_1 \varphi
    \gteq{\sigma}{\xi[\subst{\alpha}{(X,\ge)}]}{\omega[\subst{\alpha}{\varphi}]}
    t_2 \varphi$,
  \item ${\gteq{\varphi\psi}{\xi}{\omega}} =
    {\gteq{\varphi}{\xi}{\omega}}(\omega(\psi),
    \gteq{\psi}{\xi}{\omega})$,
  \item
    $\gteq{\lambda(\alpha:\kappa)\varphi}{\xi}{\omega}(\psi,) =
    \gteq{\varphi}{\xi[\subst{\alpha}{X}]}{\omega[\subst{\alpha}{\psi}]}$
    for closed $\psi \in \Tc_\kappa$ and $X \in \Cb_\psi$ and .
  \end{itemize}
  In the above, if e.g.~$\val{\psi}{\xi}{\omega} \notin
  \Cb_{\omega(\psi)}$ then $\val{\varphi \psi}{\xi}{\omega}$ is
  undefined.

  We set ${\gteq{}{\xi}{\omega}} =
  \bigcup_\sigma{\gteq{\sigma}{\xi}{\omega}}$.
\end{definition}

Note that if $t \in \val{\sigma}{\xi}{\omega}$ then $t :
\omega(\sigma)$.

\begin{lemma}\label{lem_geq_candidate}
  If $\sigma$ is a type constructor, $\omega$ is closed for~$\sigma$,
  and $\xi$ is an $\omega$-valuation, then
  $\gteq{\sigma}{\xi}{\omega}$ is a comparison candidate
  for~$\val{\sigma}{\xi}{\omega}$.
\end{lemma}

\begin{proof}
  Induction on~$\sigma$. If $\sigma=\alpha$ then $\xi(\alpha) =
  (\val{\alpha}{\xi}{\omega},\gteq{\alpha}{\xi}{\omega})$ and
  $\gteq{\alpha}{\xi}{\omega}$ is a comparison candidate
  for~$\val{\alpha}{\xi}{\omega}$ by the definition of an
  $\omega$-valuation.

  First note that property~4 of comparison candidates follows from
  Lemma~\ref{lem:liftgreater} and property~1.

  If $\sigma=\nat$ then ${\gteq{\nat}{\xi}{\omega}} = \succeq_\nat$
  then the properties of a comparison candidates for~$\val{\nat}{}{} =
  \Iterms_\nat$ follow from definitions.

  Assume $\sigma=\sigma_1\arrtype\sigma_2$. We check the first three
  properties of a comparison candidate
  for~$\val{\sigma}{\xi}{\omega}$.
  \begin{enumerate}
  \item
  \item Follows directly from the inductive hypothesis.
  \item Let $s \in \val{\sigma_1}{\xi}{\omega}$. We have
    $\circ_{\omega(\sigma)} t_1 t_2 s \leadsto
    \circ_{\omega(\sigma_2)} (t_1 s) (t_2 s)$ and
    $\circ_{\omega(\sigma)} t_1' t_2' s \leadsto
    ...
    t_2' s$ by property~1 of comparison candidates. This implies
...
\begin{proof}
  Induction on~$\sigma$. If $\sigma = \alpha$ is a type constructor
  variable then $\val{\alpha}{\xi}{\omega} = \xi(\alpha)$ is a
  candidate. If $\sigma = \nat$ then this follows from
  definitions.

  Assume $\sigma=\sigma_1\arrtype\sigma_2$. We check the properties of
  candidates from Definition~\ref{def_wm_candidate}.
  \begin{enumerate}
  ...
  \item Let $t_1,t_2 \in \val{\sigma}{\xi}{\omega}$. We need to show
    ...
    Let $s \in \val{\sigma_1}{\xi}{\omega}$ and $s :
    \omega(\sigma_1)$. Then $\circ_{\omega(\sigma)} t_1 t_2 s \leadsto
    ...
    \val{\sigma_2}{\xi}{\omega}$ by property~1 of candidates.

    Let $s_1 \succeq s_2$ and $s_i \in \val{\sigma_1}{\xi}{\omega}$
    and $s_i : \omega(\sigma_1)$. Because $t_j \in
    \val{\sigma_1\arrtype\sigma_2}{\xi}{\omega}$, we have $t_j s_i \in
    \val{\sigma_2}{\xi}{\omega}$ and $t_j s_1 \succeq t_j s_2$. Thus
    $\circ_{\omega(\sigma_2)} (t_1 s_1) (t_2 s_1) \succeq
    \circ_{\omega(\sigma_2)} (t_1 s_2) (t_2 s_2)$ by
    Lemma~\ref{lem:plustimesmonotonic} and the transitivity
    of~$\succeq$. This suffices by Lemma~\ref{lem_succ_red}.
  \item Let $t : \nat$. Then $\lift_{\omega(\sigma)} t :
    \omega(\sigma)$.

    Let $s \in \val{\sigma_1}{\xi}{\omega}$ and $s :
    \omega(\sigma_1)$. Then $\lift_{\omega(\sigma)}t s \leadsto
    \lift_{\omega(\sigma_2)} t$. By the inductive hypothesis
    $\lift_{\omega(\sigma_2)} t \in
    \val{\sigma_2}{\xi}{\omega}$. Hence $\lift_{\omega(\sigma)}t s \in
    \val{\sigma_2}{\xi}{\omega}$ by property~1 of candidates.

    Let $s_1,s_2 \in \val{\sigma_1}{\xi}{\omega}$ and $s_i :
    \omega(\sigma_1)$. We have $\lift_{\omega(\sigma_2)} t \succeq
    \lift_{\omega(\sigma_2)} t$ by the reflexivity of~$\succeq$. This
    suffices by Lemma~\ref{lem_succ_red}, because
    $\lift_{\omega(\sigma)}t s_i \leadsto \lift_{\omega(\sigma_2)} t$.
  \end{enumerate}
...
\begin{proof}
  ...
  Lemma~\ref{lem_val_wm_computable} and property~2 of candidates. Let
  $t_2' \in \val{\sigma}{\xi}{\omega}$ be such that $t_2 \succeq
  t_2'$. By Lemma~\ref{lem:plustimesmonotonic} we have
  $\circ_{\omega(\sigma)} t_1 t_2 \succeq \circ_{\omega(\sigma)} t_1
  t_2'$. This shows $\circ_{\omega(\sigma)} t_1 \in
  \val{\sigma\arrtype\sigma}{\xi}{\omega}$.

  Let $t_1' \in \val{\sigma}{\xi}{\omega}$ be such that $t_1 \succeq
  t_1'$. Let $u \in \World_{\omega(\sigma)}$. By
  Lemma~\ref{lem:plustimesmonotonic} we have $\circ_{\omega(\sigma)}
  t_1 u \succeq \circ_{\omega(\sigma)} t_1' u$. This implies
  $\circ_{\omega(\sigma)} t_1 \succeq \circ_{\omega(\sigma)} t_1'$ by
...
\begin{proof}
  By Lemma~\ref{lem:liftgreater} we have $\lift_{\omega(\sigma)}s_1
  \succeq \lift_{\omega(\sigma)}s_2$ for all $s_i : \nat$ with $s_1
  \succeq s_2$. It remains to show that $\lift_{\omega(\sigma)}s \in
  \val{\sigma}{\xi}{\omega}$ for all $s : \nat$. This follows from
  Lemma~\ref{lem_val_wm_computable} and property~3 of candidates.
\end{proof}
...
\begin{proof}
  Follows from definitions and Lemma~\ref{lem_flatten_succ}.
\end{proof}
...
\begin{proof}
  ...
  Lemma~\ref{lem_val_subst}. The only difference is that in the case
  $\sigma = \sigma_1\arrtype\sigma_2$ we need to show that if e.g.~$t
  \in \val{\sigma[\subst{\alpha}{\tau}]}{\xi}{\omega}$ and $s_1
  \succeq s_2$ and $s_i \in \val{\sigma_1}{\xi'}{\omega'}$ then $t s_1
  \succeq t s_2$. But then $s_i \in
  \val{\sigma_1[\subst{\alpha}{\tau}]}{\xi}{\omega}$ by the inductive
  hypothesis, so $t s_1 \succeq t s_2$ by the definition
  of~$\val{}{}{}$.
\end{proof}
...
For two replacements $\delta_1,\delta_2$ (see
Definition~\ref{def_closure}) we write $\delta_1 \succeq \delta_2$ iff
$\delta_1(x) \succeq \delta_2(x)$ for every~$x$.

\begin{lemma}\label{lem_typable_wm_computable}
  Assume $t : \sigma$ and $\delta_1=\gamma_1\circ\omega$,
  $\delta_2=\gamma_2\circ\omega$ are replacements such that $\delta_1
  \succeq \delta_2$ and $\omega$ is closed for~$\sigma$ and
  $\FTV(\omega(t)) = \emptyset$ and for all $x^\tau \in \FV(t)$ we
  have $\delta_i(x) \in \val{\tau}{\xi}{\omega}$. Then $\delta_i(t)
  \in \val{\sigma}{\xi}{\omega}$ and $\delta_1(t) \succeq
  \delta_2(t)$.
\end{lemma}
...
\begin{proof}
  ...
  \item If $t = x_1^{\tau_1}$ and $\tau_1 =_\beta \sigma'$ then
    ...
    Lemma~\ref{lem_beta_val_wm}. Also $\delta_1(t) \succeq
    \delta_2(t)$ by assumption.
  \item If $t = n$ is a natural number and $\sigma' = \nat$ then
    $\delta_i(t) = t$ and thus $t \in \val{\nat}{}{}$ and $\cl_1(t)
    \succeq \cl_2(t)$ by definition and the reflexivity of~$\succeq$.
  \item If $t$ is a function symbol then the claim follows from
    Lemma~\ref{lem_wm_circ}, Lemma~\ref{lem_wm_lift} or
    Lemma~\ref{lem_wm_flatten}, and the reflexivity of~$\succeq$.
  \item If $t = \abs{x:\sigma_1}{u}$ then $\sigma' =
    \sigma_1\arrtype\sigma_2$ and $u : \sigma_2$. Let $s \in
    \val{\sigma_1}{\xi}{\omega}$ and $s : \omega(\sigma_1)$. Let
    $\delta_i'=\delta_i[\subst{x}{s}]$. This is well-defined because
    $\omega(x)$ has type~$\omega(\sigma_1)$ We have $\delta_1' \succeq
    \delta_2'$ by the reflexivity of~$\succeq$. Hence by the inductive
    ...
    Let $s_1,s_2 \in \val{\sigma_1}{\xi}{\omega}$ be such that $s_i :
    \omega(\sigma_1)$ and $s_1 \succeq s_2$. Let
    $\delta_i'=\delta_i[\subst{x}{s_i}]$. We have $\delta_1 \succeq
    \delta_2$. Hence by the inductive hypothesis
    $\delta_1'(u)\succeq\delta_2'(u)$. We have $\delta_i(\abs{x}{u})
    s_i \leadsto \delta_i'(u)$. Thus $\delta_1(t) s_1 \succeq
    \delta_2(t) s_2$ by Lemma~\ref{lem_succ_red}.
  \item If $t = \tabs{\alpha:\kappa}{u}$ then $\sigma' =
    ...
    \succeq \delta_2'(u)$. We have $\delta_i(\tabs{\alpha}{u}) \psi
    \leadsto \delta_i'(u)$. Hence $\delta_i(\tabs{\alpha}{u}) \psi \in $.
  \item If $t = t_1 t_2$ then $t_1 : \tau\arrtype\sigma'$ and $t_2 :
    ...
    $\omega^*(t_1) \in \val{\tau\arrtype\sigma'}{\xi}{\omega}$ and
    $\omega^*(t_2) \in \val{\tau}{\xi}{\omega}$. We have
    $\omega^*(t_2) : \omega(\tau)$. Then by definition $\omega^*(t) =
...
\item Note that the type interpretation $\typeinterpret{\mathtt{or}\,
  \rho\,\pi}$ evaluates to $\quant{\alpha}{\quant{\beta}{((\sigma
  \arrtype \alpha) \arrtype (\tau \arrtype \alpha) \arrtype\beta)
  \arrtype \beta} \arrtype \alpha}$.  I will not fill that one in in
  the formula, but note that it is the output type of the inner
  $\mathtt{case}$.
\item Note that $\Termmap(\mathtt{case}) = \Lambda \alpha\beta\xi.
  \lambda x,y,z.x * \xi \cdot (\Lambda \chi \lambda f.f \cdot y \cdot
  z) \oplus (y \cdot \lift(0)) \oplus (z \cdot \lift(0)) \oplus
  \lift(7)$.
\item Let's fill in the inner $\mathtt{case}$ interpretations in the
  ...
  \mathcal{J}(\mathtt{case})_{\sigma,\tau,\xi}(\interpret{u},
    ...
    ((\abs{z}{\interpret{v}}) \cdot \lift(0)) \oplus
    ((\abs{a}{\interpret{w}}) \cdot \lift(0)) \oplus \lift(7)}, \\
    \phantom{ABCDEFGHIJ}
...
\item And $\beta$-normalise them.  We're allowed to do that by
  ...
  \mathcal{J}(\mathtt{case})_{\sigma,\tau,\xi}(\interpret{u},
    ...
    \interpret{w}[a:=\lift(0)] \oplus \lift(7)}\ )
  $
\item Now the inner $\mathtt{case}$ interpretation in the left-hand
  side: \\
  $\mathcal{J}(\mathtt{case})_{\rho,\pi,\xi}(
  ...
  \mathcal{J}(\mathtt{case})_{\sigma,\tau,\xi}(\interpret{u},
    ...
    \interpret{w}[a:=\lift(0)] \oplus \lift(7)}\ )
  $
\item Now it gets messy. I will fill in the outer
  $\mathtt{case}$ interpretation on the left. \\
  $\langle\ \  \interpret{u} * \typeinterpret{\mathtt{or}\,\rho\,\pi} \cdot
    ...
    \phantom{A} * \xi \cdot (\Lambda \chi\lambda f.f \cdot
    (\abs{z}{\interpret{v}}) \cdot (\abs{a}{\interpret{w}})) \\
    \oplus
    \interpret{v}[z:=\lift(0)] \oplus \interpret{w}[a:=\lift(0)]
    \oplus \lift(7) \red \\
  \mathcal{J}(\mathtt{case})_{\sigma,\tau,\xi}(\interpret{u},
    \abs{x}{\interpret{s} * \xi \cdot (\Lambda \chi.\lambda f.f \cdot
    (\abs{z}{\interpret{v}}) \cdot (\abs{a}{\interpret{w}})) \oplus \\
    ...
    \interpret{w}[a:=\lift(0)] \oplus \lift(7)}\ )
  $
\item And of course also on the right. \\
  $\langle\ \  \interpret{u} * \typeinterpret{\mathtt{or}\,\rho\,\pi} \cdot
    (\Lambda \chi\lambda f.f \cdot (\abs{x}{\interpret{s}}) \cdot
    (\abs{y}{\interpret{t}})) \oplus \\
    \phantom{Ai}
  ...
  \interpret{u} * \xi \cdot
    ...
    \phantom{A}
    \interpret{v}[z:=\lift(0)] \oplus
    \interpret{w}[a:=\lift(0)] \oplus \lift(7)\ \oplus \\
