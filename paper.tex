...
\begin{definition}
%We assume given a fixed set $\Sigma_{\mathsf{poly}}$ of \emph{function
%symbols}, each indexed with $0$ or more types variables and paired with
%an arity $k$ as well as $k$ input types and one output type, denoted
...
%\alpha_n\}$.
%We let $\Sigma$ denote the set of all \emph{instanced} symbols, so
%$\{ \mathtt{f}_{\gamma(\alpha_1),\dots,\gamma(\alpha_n)} : [\sigma_1
%\gamma \times \dots \times \sigma_k\gamma] \arrtype \tau\gamma \mid
%\mathtt{f}_{\alpha_1,\dots,\alpha_n} : \sigma_1 \times \dots \times
%\sigma_k] \arrtype \tau \in \Sigma_{\mathsf{poly}} \wedge \gamma$ a
%type substitution$\}$.
We assume given a fixed set $\Sigma$ of \emph{function symbols}, each
indexed with $0$ or more types and paired with an arity $k$ as well as
$k$ input types and one output types, denoted $\mathtt{f}_{\rho_1,
\dots,\rho_n} : [\sigma_1 \times \dots \times \sigma_k] \arrtype \tau$;
we require that for all symbols $\mathtt{f}_{\rho_1,\dots,\rho_n} :
[\rho_1 \times \dots \times \rho_k] \arrtype \sigma \in \Sigma$:
\begin{itemize}
\item there is no other type declaration $D$ such that also
  $\mathtt{f}_{\rho_1,\dots,\rho_n} : D \in \Sigma$;
\item for all type substitutions $\gamma$, also $\mathtt{f}_{\rho_1
  \gamma,\dots,\rho_n\gamma} : [\sigma_1\gamma \times \dots \times
  \sigma_k\gamma] \arrtype \tau\gamma \in \Sigma$.
\end{itemize}
...
\begin{itemize}
\item $\Gamma \vdash x : \sigma$ for every $(x : \sigma) \in \Gamma$.
\item $\Gamma \vdash \mathtt{f}_{\vec{\rho}}(s_1,\dots,s_k) : \tau$ if
  $(\mathtt{f}_{\vec{\rho}} : [\sigma_1 \times \dots \times \sigma_k]
  \arrtype \tau) \in \Sigma$ and $\Gamma \vdash s_i : \sigma_i$ for $1
  \leq i \leq k$
\item $\Gamma \vdash \abs{x:\sigma}{s} : \sigma \arrtype \tau$ if $x
  \in \Vars$ and $\Gamma \uplus \{ x : \sigma \} \vdash s : \tau$.
\item $\Gamma \vdash \tabs{\alpha}{s} : \quant{\alpha}{\sigma}$ if
...
by including a symbol $@_{\sigma,\tau} : [\sigma \arrtype \tau \times
\sigma] \arrtype \tau$ in $\Sigma_{\mathsf{poly}}$.
Similarly, type application is modelled through a symbol
$\mathtt{A}_{\quant{\alpha}{\sigma},\tau} : [\quant{\alpha}{\sigma}]
\arrtype \sigma[\alpha:=\tau]$.
...
\begin{definition}
We assume given a set $\Rules$ of triples $(\Gamma,\ell,r)$ of a
variable environment and two terms, such that:
\begin{itemize}
\item $\FV(r) \subseteq \FV(\ell) \subseteq \mathit{keys}(\Gamma)$;
\item $\ell$ and $r$ have the same type under $\Gamma$;
\item $\FTV(\ell) = \FTV(r) = \emptyset$, and all type variables in
  the range of $\Gamma$ are bound;
\item $\Rules$ is stable: if $(\Gamma,\ell,r) \in \Rules$ and $\gamma$
  is a substitution, then $(\Delta,\ell\gamma,r\gamma) \in \Rules$ for
  appropriate $\Delta$. (\CK{Appropriate depends on the typing of
  $\gamma$ of course.})
\end{itemize}
Fixing a variable environment $\Gamma$, the reduction relation
$\arr{\Rules}$ is the smallest monotonic relation that contains all
pairs $(s,t)$ that are well-typed under $\Gamma$, such that some
restriction $\Gamma'$ of $\Gamma$ exists with $(\Gamma',s,t) \in
\Rules$.
\end{definition}
...
\[
\begin{array}{rclrcl}
\epsilon_\tau & : & [\bot] \arrtype \tau &
\pi^1_{\sigma,\tau} & : & [\mathtt{and}(\sigma,\tau)] \arrtype \sigma \\
@_{\sigma,\tau} & : & [\sigma \arrtype \tau \times \sigma] \arrtype \tau &
\pi^2_{\sigma,\tau} & : & [\mathtt{and}(\sigma,\tau)] \arrtype \tau \\
\mathtt{in}^1_{\sigma,\tau} & : & [\sigma] \arrtype
  \mathtt{or}(\sigma,\tau) &
\mathtt{pair}_{\sigma,\tau} & : & [\sigma \times \tau] \arrtype
  \mathtt{and}(\sigma,\tau)) \\
\mathtt{in}^2_{\sigma,\tau} & : & [\tau] \arrtype
  \mathtt{or}(\sigma,\tau) &
\mathtt{tapp}_{\quant{\alpha}{\sigma},\tau} & : &
  [\quant{\alpha}{\sigma}] \arrtype \sigma[\alpha:=\tau] \\
\mathtt{case}_{\sigma,\tau,\rho} & : & [\mathtt{or}(\sigma,\tau) \times
  \sigma \arrtype \rho \times \tau \arrtype \rho] \arrtype \rho\ \ \ & 
\mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\rho} & : &
  [\qquant{\exists}{\alpha}{\sigma} \times 
  \quant{\alpha}{\sigma \arrtype \rho}] \arrtype \rho) \\
\mathtt{ext}_{\qquant{\exists}{\alpha}{\sigma},\tau} & : &
  [\sigma[\alpha:=\tau]] \arrtype \qquant{\exists}{\alpha}{\sigma}
  & & & \phantom{x}\hfill
  \text{only for}\ \rho\ \text{with}\ \alpha \notin \FTV(\rho) \\
\end{array}
\]
...
done on the indexes of function symbols (thus giving essentially
...
\mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\rho}(
  \mathtt{ext}_{\qquant{\exists}{\alpha}{\sigma},\tau}(s),\tabs{\alpha}{
...
\mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\rho}(\epsilon_{\qquant{
  \exists}{\alpha}{\sigma}}(s),\tabs{\alpha}{\abs{x}{t}}) & \to &
  \epsilon_\rho(s) \\
\end{array}
\]
\begin{itemize}
...
\item $
\mathtt{let}_{\qquant{\exists}{\alpha}{\rho}}(
  \mathtt{case}_{\sigma,\tau,\qquant{\exists}{\alpha}{\rho}}(
  u,\abs{x}{s},\abs{y}{t}),v) %& \to &
  \to\\
  \mathtt{case}_{\sigma,\tau,\rho}(u,
  \abs{x}{\mathtt{let}_{\qquant{\exists}{\alpha}{\rho}}(s,v)},
  \abs{y}{\mathtt{let}_{\qquant{\exists}{\alpha}{\rho}}(t,v)})
  $%\\
%\end{array}
%\]
\end{itemize}
\begin{itemize}
\item $\epsilon_\tau(\mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},
  \bot}(s,t)) \to
  \mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\tau}(s,\epsilon_\tau(t))$
\item $@_{\tau,\rho}(\mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},
  \tau \arrtype \rho}(s,t),u) \to
  \mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\rho}(s,@_{\tau,\rho}(t,
  u))$
\item $\mathtt{tapp}_{\quant{\alpha}{\tau},\rho}(
\mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\quant{\alpha}{\tau}}(s,t))
  \to
  \mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\tau[\alpha:=\rho]}(s,
  \mathtt{tapp}_{\quant{\alpha}{\tau},\rho}(t))$
\item $\pi^1_{\tau,\rho}(\mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},
  \mathtt{and}(\tau,\rho)}(s,t)) \to
  \mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\tau}(s,\pi^1_{\tau,
  \rho}(t))$
\item $\pi^2_{\tau,\rho}(\mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},
  \mathtt{and}(\tau,\rho)}(s,t)) \to
  \mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\rho}(s,\pi^2_{\tau,
  \rho}(t))$
\item $\mathtt{case}_{\tau,\rho,\pi}(
  \mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\mathtt{or}(\tau,
  \rho)}(s,t),\abs{x}{u},\abs{y}{v}) \to
  \mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\pi}(s,\mathtt{case}_{
  \tau,\rho,\pi}(t,\abs{x}{u},\abs{y}{v}))$
\item $\mathtt{let}_{\qquant{\exists}{\beta}{\tau},\rho}(\mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\qquant{\exists}{\beta}{\tau}}(s,t),u) \to
  \mathtt{let}_{\qquant{\exists}{\alpha}{\sigma},\rho}(s,\mathtt{let}_{\qquant{\exists}{\beta}{\tau},\rho}(t,u))$
\end{itemize}
...
All terms will be mapped to \emph{tuples} of types and interpretation
terms, the latter of which can be compared using $\succ$ to hopefully
obtain a decrease that can be used with rule removal.
...
$\mathtt{f} : [\sigma_1 \times \dots \times \sigma_n] \arrtype
\tau \in \Sigma$ an interpretation term $\Termmap(\mathtt{f})$ of the
form $\abs{x_1:\Typemap(\sigma_1) \dots x_n:\Typemap(\sigma_n)}{t}$,
such that a type substitution $\gamma$ exists with $\emptyset \vdash
\Termmap(\mathtt{f}) : \Typemap(\sigma)\gamma$.
...
\begin{lemma}
If $s : \sigma$ then $\interpret{s} : \Typemap(\sigma)$ is a well-typed
interpretation term.
\end{lemma}
