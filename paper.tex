...
\item $\Rules$ is stable: if $(\Gamma,\ell,r) \in \Rules$ and $\gamma$
  ...
  $\gamma$ of course.})
\end{itemize}
The reduction relation $\arr{\Rules}$ is the monotonic closure of
$\Rules$.
\end{definition}
...
\TypeQuantifiers = \{\quad
  \mathtt{forall} : 1,\quad
  \mathtt{exists} : 1\quad
  \} \\
\end{array}
\]
...
\[
\begin{array}{rcl}
\epsilon_\tau & : & (1,\ \bot \arrtype \tau) \\
@_{\sigma,\tau} & : & (2,\ (\sigma \arrtype \tau) \arrtype \sigma
  \arrtype \tau) \\
\mathtt{in}^1_{\sigma,\tau} & : & (1,\ \sigma \arrtype
  \mathtt{or}(\sigma,\tau)) \\
\mathtt{in}^2_{\sigma,\tau} & : & (1,\ \tau \arrtype
  \mathtt{or}(\sigma,\tau)) \\
\mathtt{case}_{\sigma,\tau,\rho} & : & (3,\ \mathtt{or}(\sigma,\tau)
  \arrtype (\sigma \arrtype \rho) \arrtype (\tau \arrtype \rho)
  \arrtype \rho) \\
\mathtt{pair}_{\sigma,\tau} & : & (2,\ \sigma \arrtype \tau \arrtype
  \mathtt{and}(\sigma,\tau)) \\
\pi^1_{\sigma,\tau} & : & (1,\ \mathtt{and}(\sigma,\tau) \arrtype
  \sigma) \\
\pi^2_{\sigma,\tau} & : & (1,\ \mathtt{and}(\sigma,\tau) \arrtype \tau) \\
\mathtt{all}_{\quant{\alpha}{\sigma}} & : & (1,\ 
  \quant{\alpha}{\sigma} \arrtype \mathtt{forall}(\quant{\alpha}{
  \sigma})) \\
\mathtt{tapp}_{\quant{\alpha}{\sigma},\tau} & : &
  (1,\ \mathtt{forall}_{\quant{\alpha}{\sigma}} \arrtype
  \sigma[\alpha:=\tau]) \\
\mathtt{ext}_{\quant{\alpha}{\sigma},\tau} & : &
  (1,\ \sigma[\alpha:=\tau] \arrtype \mathtt{exists}(\quant{\alpha}{
  \sigma})) \\
\mathtt{let}_{\quant{\alpha}{\sigma},\rho} & : &
  (2,\ \mathtt{exists}(\quant{\alpha}{\sigma}) \arrtype 
...
\mathtt{tapp}_{\quant{\alpha}{\sigma},\tau}(
  \mathtt{all}_{\quant{\alpha}{\sigma}}(\tabs{\alpha}{s})) & \to &
  s[\alpha:=\tau] \\
...
\mathtt{let}_{\quant{\alpha}{\sigma},\rho}(
  \mathtt{ext}_{\quant{\alpha}{\sigma},\tau}(s),\tabs{\alpha}{\abs{
    x}{t}}) & \to & t[\alpha:=\tau][x:=s] \\
\\
\epsilon_\tau(\epsilon_\bot(s)) & \to & \epsilon_\tau(s) \\
@_{\sigma,\tau}(\epsilon_{\sigma \arrtype \tau}(s),t) & \to &
  \epsilon_\tau(s) \\
\mathtt{tapp}_{\quant{\alpha}{\sigma},\tau}(
  \epsilon_{\mathtt{forall}(\quant{\alpha}{\sigma})}(s)) & \to &
...
\mathtt{let}_{\quant{\alpha}{\sigma},\rho}(\epsilon_{\mathtt{exists}(
  \quant{\alpha}{\sigma})}(s),\tabs{\alpha}{\abs{x}{t}}) & \to &
  \epsilon_\rho(s) \\
\\
\epsilon_\rho(\mathtt{case}_{\sigma,\tau,\bot}(u,\abs{x}{s},\abs{y}{t}))
  & \to & \mathtt{case}_{\sigma,\tau,\rho}(u,\abs{x}{\epsilon_\rho(s)},
  \abs{y}{\epsilon_\rho(t)}) \\
@_{\rho,\pi}(\mathtt{case}_{\sigma,\tau,\rho \arrtype \pi}(u,
  \abs{x}{s},\abs{y}{t}),v) & \to & \mathtt{case}_{\sigma,\tau,\pi}(u,
