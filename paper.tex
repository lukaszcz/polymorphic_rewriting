...
%\begin{lemma}\label{lem_eta_reduce_substitution}
%Let $\gamma$ be a substitution mapping variables $x$ to terms $\gamma(
%x)$ such that $y \arrnormalise^* \gamma(x)$ for some variable $y$ not
%in the domain of $\gamma$.
%Let $s$ and $t$ be interpretation terms such that $s$ is linear,
%$s\gamma \arrnormalise t$ and $\FV(s)$ does not contain variables in
%the range of $\gamma$.
%Then there exist $t'$ and a substitution $\delta$ such that:
%(1) $s \arrnormalise^* t'$;
%(2) $t = t'\delta$;
%(3) $\gamma \arrnormalise^* \delta$ (that is, $\delta$ has the same
%  domain as $\gamma$ and each $\gamma(x) \arrnormalise^* \delta(x)$).
%\end{lemma}
%
%\begin{proof}
%By induction on the form of $s$, using Lemma~\ref{lem_abusive_notation}
%for case analysis.
%
%First observe that $s$ is not a function symbol, as then $s\gamma = s$
%does not reduce.
%
%Next, suppose that $s$ is a variable $x$.  If $x$ is not in the domain
%of $\gamma$, then $s = s\gamma \arrnormalise t$, and since
%$\arrnormalise$ cannot introduce fresh unbound variables, $t = t\gamma$
%as well; thus, we are done choosing $t' := t$ and $\delta := \gamma$.
%If $x$ is in the domain of $\gamma$, then we let $t' := x$ (which surely
%gives $s \arrnormalise^* t'$) and take $\delta := [x:=t] \cup [y:=
%\gamma(y) \mid y \in \mathtt{domain}(\gamma) \setminus \{x\}]$.  This
%choice satisfies all properties.
%
%Now suppose $s = \abstraction{a}{u}$.  Then by $\alpha$-conversion
%we can safely assume that $a$ does not occur in domain or range of
%$\gamma$, so $s\gamma = \abstraction{a}{(u\gamma)}$.
%Clearly $t = \abstraction{a}{v}$ with $u\gamma \arrnormalise v$.  By the
%induction hypothesis, we obtain $v'$ and $\delta$ such that $u
%\arrnormalise v'$ and $v = v'\delta$ and $\gamma \arrnormalise^*
%\delta$..  Since $\delta$ has the same domain as $\gamma$, $a$ does not
%appear in it, and since $\arrnormalise$ cannot introduce fresh variables
%or type variables, $a$ also does not appear in the range of $\delta$.
%Thus, $t = (\abstraction{a}{v'})\delta$, and we indeed have $s =
%\abstraction{a}{u} \arrnormalise \abstraction{a}{v'}$.
%
%The only remaining case is when $s = u t_1 \ldots t_n$ with $n > 0$ and
%$u$ not an application.  If $t = (u\gamma) (t_1\gamma) \ldots v_i \ldots
%(t_n\gamma)$ with $t_i\gamma \arrnormalise v_i$ then the induction
%hypothesis provides a suitable $\delta$ and $v_i'$ such that $v_i =
%v_i'\delta$ and $t_i \arrnormalise v_i'$; we are done choosing $t :=
%u t_1 \ldots v_i' \ldots t_n$. NOPE
%\end{proof}

%\begin{lemma}
%Let $\gamma$ be a substitution mapping variables $x$ to terms $t$ such
%that $x \arrnormalise^** t$.  If $s \in \SN$ then also $s\gamma \in
%\SN$.
%\end{lemma}
%
%\begin{proof}
%We prove that $s\gamma \in \SN$, ordered by $s$ using the union of
%$\arrnormalise$ and the subterm relation (which is well-founded on
%terms in $\SN$).  We note that $s\gamma$ is terminating if all its
%reducts are terminating, so suppose $s\gamma \arrnormalise t$.  To
%prove that $t \in \SN$, we consider the form of $s$, using
%Lemma~\ref{lem_abusive_notation}.
%
%First, if $s = \abstraction{a}{s'}$, then $t = \abstraction{a}{t'}$
%with $s'\gamma \arrnormalise t'$ (since, by $\alpha$-conversion, we
%can safely assume that $a$ does not occur in the domain or range of
%$\gamma$).  By induction hypothesis, $t' \in \SN$, so by
%Lemma~\ref{lem_reduce_abs} so is $t$.
%
%Second, if $s$ is neither an abstraction nor an application, then
%there are two possibilities: $s$ is a function symbol -- in which
%case $s\gamma = s \in \SN$ by assumption -- or $s$ is a variable --
%in which case $s\gamma \arrnormalise^* t$ whether or not $s$ is in
%the domain of $\gamma$.  Since $s \in \SN$ by Lemma~\ref{lem_var_sn},
%the same holds for its reducts.
%
%Third, if $s$ is an application $s' t_1 \ldots t_n$ with $n > 0$ and
%$t$ has the form $(s\gamma) t_1 \dots t_i' \dots t_n$ with $s_i\gamma
%\arrnormalise t_i'$, or the form $s'' t_1 \ldots t_n$ with $s'\gamma
%\arrnormalise s''$, then
%\end{proof}

%\begin{lemma}\label{lem_circ_sn}
%Let $t_1,\dots,t_n \in \SN$.  Then $\apps{\circ_\sigma}{t_1}{t_n} \in
%\SN$ for $\circ \in \{\oplus,\otimes\}$
%and $\sigma := \sigma_3 \arrtype \dots \arrtype \sigma_n \arrtype \tau$.
%\end{lemma}
%
%\begin{proof}
%TODO
%
%We prove the lemma by induction first on the size of $\tau$, second on
%$(t_1,\dots,t_n)$ ordered by the product extension of $\arrnormalise$.
%We are done if all reducts of $\apps{a}{t_1}{t_n}$ are in $\SN$, where
%$a$ is $\circ_\sigma$ or $x$.  Consider what forms such a reduct $t$ may
%have:
%\begin{itemize}
%\item A reduction by $\arreta$ at the root produces
%  $t := \abs{x}{(\app{\apps{a}{t_1}{t_n}}{x})}$ for a fresh variable $x$
%  of type $\tau_1$, where $\tau$ is $\tau_1 \arrtype \tau_2$.
%  Variables have the required form to apply (\ref{lem_base_sn:circ})
%  (with $n=0$), and $\tau_1$ is smaller than $\tau$, so $x$ is in $\SN$.
%  Thus, $t' := \app{\apps{a}{t_1}{t_n}}{x}$ satisfies the requirements
%  to apply the induction hypothesis (since $\Gamma \cup \{x:\tau_1\}
%  \proves t' : \tau_2$), and if $t'$ is terminating then so is $t$ by
%  Lemma~\ref{lem_reduce_abs}.
%\item A reduction by $\normstep$ at the root can only apply if $n = 0$
%  and $\sigma = \tau = \nat$; then $t \in \Nbb$, so $t$ is in normal
%  form.
%\item A reduction by $\normstep$ at the head
%\end{itemize}
%\end{proof}
