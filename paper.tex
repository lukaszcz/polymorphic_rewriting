...
\item $\gamma \Vdash s\ R_{\sigma \arrtype \tau} t$ if:
  ...
  \item $s = \abs{x:\sigma}{s'}$ and $t = \abs{x:\sigma}{t'}$ and
    ...
    interpretation terms of type $\sigma$
...
\item $\gamma \Vdash s\ R_{\quant{\alpha}{\sigma}}\ t$ if:
  ...
  \item $s = \tabs{\alpha}{s'}$ and $t = \tabs{\alpha}{t'}$ and
    $s'[\subst{\alpha}{\tau}]\ R_\sigma\ t'[\subst{\alpha}{\tau}]$ for
    all types $\tau$
...
simply assume that typical systems have infinitely many rules (e.g., all
instances of $\beta$-reductions are a separate rule)?  We're already
using infinite signatures -- should we head straight for polymorphism?
