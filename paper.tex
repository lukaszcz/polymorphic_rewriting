...
\begin{definition}\label{def:itypes}\normalfont
  ...
  \item If $\alpha$ is a type constructor variable of kind~$\kappa$
    ...
    \CK{Ah, I think I wrote this early and then later saw the usage as
      I saw that substitution was also extended (I think I wrote this
      in an earlier version, before you removed the definition of type
      substitution). You can disregard.}
  ...
  \CK{I don't think it matters much, but if we have just two it would
    be easy to use a macro.}
...
\CK{Well, if we do the restructuring thing, then I would guess this
  is very easily done: we first introduce the general framework, and
  then introduce \emph{interpretation} types as this specific instance
...
\CK{Agreed; it also makes it easier to apply, since I think we will
  often want to compare non-normalised terms. However, the last
  one still needs rephrasing: $\tau$ should be a type constructor of
  the same kind as $\alpha$, not a type.}
...
\CK{Note that the example above does not work if $\alpha$ has a
different kind than *; it will require the construction of a type
constructor of arbitrary given kind.}
\end{proof}
...
\CK{Correction: not closed type, but whatever the requirement in
Definition \ref{def:succ} ends up becoming.}
\end{proof}
...
\CK{Pondering: do we also have that $\lift_\sigma(n+m) = \oplus_\sigma
(\lift_\sigma n)\ (\lift_\sigma n)$? Could be useful. (But let's first
see whether it's needed for anything.)}
...
\item\label{lem:approx:neutral} $\oplus_\sigma(\CKchange{\lift_\sigma 0}) s
  \approx s$ and $\otimes_\sigma(\CKchange{\lift_\sigma 1})) s \approx s$.
\end{enumerate}
\end{lemma}
...
\CK{I think another useful property we could think about adding here is
that $\succ$ and $\succeq$ are preserved under $\leadsto$ -- so, say,
$s\ R\ t$ if and only if $s\downarrow\ R\ t\downarrow$.  (Is that true?)}
