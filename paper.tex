...
\LC{If $=$ here is the reflexive-symmetric-transitive closure
  of~$\leadsto$ \emph{and $\sigma$ is closed} then yes, this follows
...
\succ t$ then we have $s \oplus_\sigma u \succ t \oplus_\sigma u$, and
if $u \succeq \lift_\sigma(1)$ then also $s \otimes_\sigma u \succ t
\otimes_\sigma u$.
\end{lemma}
...
\CK{If $\succ$ and $\succeq$ are \emph{monotonic} (i.e., if $s \succ t$
(resp.\ $s \succeq t$) then $C[s] \succ C[t]$ (resp.\ $C[s] \succeq C[t]$)
for all contexts $C$), this} gives rise to the following algorithm:
\begin{enumerate}
...
\begin{lemma}
Suppose that for all symbols
$\afun : \forall (\alpha_1 : \kappa_1) \ldots
  \forall (\alpha_n : \kappa_n) . \sigma_1 \arrtype \ldots \arrtype
  \sigma_k \arrtype \tau$ we have:
$\Termmap(\afun) = \tabs{\alpha_1 \dots \alpha_n}{\abs{x_1 \dots x_k}{
  s_1 \oplus_{\interpret{\tau}} \dots \oplus_{\interpret{\tau}} s_m}}$
  such that for all $1 \leq i \leq k$ there exists $1 \leq j \leq m$
  such that $s_j$ has one of the forms:
  \begin{itemize}
  \item $x_i \cdots$ (so $x_i$ or an application with $x_i$ at the head);
  \item $\lift_{\interpret{\tau}}(\flatten_{\interpret{\sigma_i}}(x_i) +
    n)$ for some $n$ (in the case of $0$, the addition may be omitted);
  \end{itemize}
Then both $\succinterpret$ and $\succeqinterpret$ are monotonic.
\end{lemma}
...
\CK{Note: I am not entirely sure what to do with pairing, since we
currently do not \emph{have} pairing.  With some luck it will work
out with the two cases above, but will have to investigate how I should
adapt the interpretations that currently use pairing.}

\LC{We \emph{do} have pairing for all practical purposes, definable in
  System~$F_\omega$ with the right reduction behaviour. We can treat
  it explicitly here, if that's needed. But if $m$ can be $0$ above,
  then we have the encodings of covered. In general, we have the
  encodings of all inductive data types with one constructor
  covered. But not the encodings of lists or any inductive data type
  with more than one constructor, because then the encodings of
  constructors erase all but one argument. I think we should invent
  something to have all encodings covered, maybe use
  $\lift_{\interpret{\tau}}(\flatten_{\interpret{\sigma_i}}(x_i))$
  inside the encodings to add the erased arguments.}

\CK{\dots snipped rambling \dots}

\LC{It's not completely obvious to me how to do this, so I'd like to
  see this spelled out in detail.}

\CK{Is it more clear now?}

\LC{Yes, but I'm not sure if the lemma is true as stated.}
...
\item $@_{\sigma,\tau}(\abs{x}{s},t) \red s[x:=t]$ \\
  ...
  \ref{lem:liftgreater} we have \LC{(We do \emph{not} have this if~$\tau$ is not closed, e.g., a type variable!!)} $\lift(\flatten(\interpret{t}) \oplus
  1) \succeq \lift(1)$, by Lemma \ref{lem:plustimesmonotonic} \LC{(Again, this doesn't work if $\sigma,\tau$ are not closed)} this term
