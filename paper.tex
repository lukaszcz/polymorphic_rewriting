...
A \emph{type constructor substitution} is a partial function
$[\alpha_1:=\sigma_1, \dots,\alpha_n:=\sigma_n]$ mapping a finite set
of type variables to types.  We let
$\tau[\alpha_1:=\sigma_1,\dots,\alpha_n:=\sigma_n]$ denote $\tau$ with
all occurrences of some $\alpha_i$ replaced by the corresponding
$\sigma_i$.  We use alpha-conversion to guarantee that substitution
does not capture variables in any of the $\sigma_i$.
\end{definition}
...
\end{enumerate}
Note that the above rules are invariant under~$\equiv$ (by confluence
of $\beta$-reduction on types), so they correctly define a relation on
...
(a) $s$ is closed, and (b) $s$ is in normal form with respect to
$\leadsto$.  We let $\World$ be the set of all final interpretation
...
$\oplus_\tau$, $\otimes_\tau$, $\flatten_\tau$ and $\lift_\tau$
``match'' on the type~$\tau$. This makes adapting the computability
...
\begin{definition}\label{def_candidate}\normalfont
  ...
  define the set~$\Cb_\tau^\Gamma$ of candidates of type
  constructor~$\tau$ in context~$\Gamma$.

  First assume $\kappa=*$, i.e., $\tau$ is a type. A set~$X$ of
  interpretation terms of type~$\tau$ in context~$\Gamma$ is a
  \emph{candidate of type~$\tau$} when:
  \begin{enumerate}
  \item $X \subseteq \SN$;
  ...
  \item if $t_1,t_2 \in X$ then $\circ_\tau t_1 t_2 \in X$ for $\circ
    \in \{\oplus,\otimes\}$;
  \item if $t \in \SN$ and $\Gamma \proves t : \nat$ then
    $\lift_\sigma(t) \in X$.
  \end{enumerate}
  ...
  \Tc_{\kappa_1} \times \bigcup_\xi\Cb_\xi^\Gamma \to
  \bigcup_\xi\Cb_\xi^\Gamma$ is a \emph{candidate of type
    constructor~$\tau$} if for every type constructor~$\sigma$ of
  kind~$\kappa_1$ and a candidate $X \in \Cb_\sigma^\Gamma$ we have
  $f(\sigma,X) \in \Cb_{\tau\sigma}^\Gamma$.
\end{definition}

\begin{definition}\label{def_reducibility_valuation}\normalfont
  Let $\omega$ be a mapping from type constructor variables to type
  constructors (respecting kinds). A \emph{$\Gamma,\omega$-valuation}
  is a mapping~$\xi$ from type constructor variables to candidates
  such that $\xi(\alpha) \in \Cb_{\omega(\alpha)}^\Gamma$.

  For each type constructor~$\sigma$, context~$\Gamma$,
  mapping~$\omega$ and each $\Gamma,\omega$-valuation~$\xi$, the set
  $\val{\sigma}{\omega,\xi}{\Gamma}$ is defined by induction
  on~$\sigma$:
  \begin{itemize}
  ...
  \item $\val{\sigma \arrtype \tau}{\omega,\xi}{\Gamma}$ is the set of all
    terms~$t$ such that $\Gamma \proves t : \omega(\sigma \arrtype \tau)$,
    and for every~$s \in \val{\sigma}{\omega,\xi}{\Gamma}$ we have
    $\app{t}{s} \in \val{\tau}{\omega,\xi}{\Gamma}$,
  \item $\val{\forall(\alpha:\kappa)[\sigma]}{\omega,\xi}{\Gamma}$ is
    the set of all terms~$t$ such that $\Gamma \proves t :
    \omega(\forall(\alpha:\kappa)[\sigma])$ and for every type
    constructor~$\tau$ of kind~$\kappa$ and every $X \in
    \Cb_\tau^\Gamma$ we have $\tapp{t}{\alpha} \in
    \val{\sigma}{\omega[\subst{\alpha}{\tau}],\xi[\subst{\alpha}{X}]}{\Gamma}$,
  \item $\val{\varphi \psi}{\omega,\xi}{\Gamma} =
    \val{\varphi}{\omega,\xi}{\Gamma}(\val{\psi}{\omega,\xi}{\Gamma})$,
  \item
    $\val{\lambda(\alpha:\kappa)\varphi}{\omega,\xi}{\Gamma}(\tau,X) =
    \val{\varphi}{\omega[\subst{\alpha}{\tau}],\xi[\subst{\alpha}{X}]}{\Gamma}$
    for $\tau \in \Tc_\kappa$ and $X \in \Cb_\tau^\Gamma$.
  \end{itemize}
\end{definition}

\begin{lemma}\label{lem_val_typable}
  If $\sigma$ is a type and $t \in \val{\sigma}{\omega,\xi}{\Gamma}$
  then $\Gamma \proves t : \omega(\sigma)$.
\end{lemma}
...
\begin{proof}
  ...
  \item Let $t_1,t_2 \in \SN$ be such that
    ...
    So $\circ_\nat t_1 t_2 \in \val{\nat}{\omega,\xi}{\Gamma}$.
  \item Let $t \in \SN$ be such that $\Gamma \proves t : \nat$. Then
    $\Gamma \proves \lift_\tau t : \nat$. It remains to show
    $\lift_\nat t \in \SN$. \ldots
  \end{enumerate}
\end{proof}
