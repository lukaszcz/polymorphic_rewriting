...
\Gamma$
%with $\FTV(\typeinterpret{\tau}) \setminus \FTV(\tau) =
%\{\beta_1,\dots,\beta_n\}$ there exist type variables $\beta_1,\dots,
%\beta_n$ such that $(x : \interpret{\tau}[\beta_1:=\alpha_1,\dots,
%\beta_n:=\alpha_n]) \in \Delta$.
we have $(x : \tau'\delta) \in \Delta$ for some $\delta$ that fills in
the blanks caused by interpretation of $\tau$.

Then there exists a type substitution $\gamma$
%on domain $\FTV(
%\typeinterpret{\sigma}) \setminus \FTV(\sigma)$ such that
that fills in the blanks caused by interpretation of $\sigma$ such that
$\Delta \vdash \interpret{s} : \typeinterpret{s}\gamma$.
\end{lemma}

=======================

We will shortly denote $[\beta_1:=\alpha_1,\dots,\beta_n:=\alpha_n]$ as
$[\vec{\beta}:=\vec{\alpha}]$.

\begin{proof}
By induction on the form of $s$.

Clearly if $s$ is a variable we are done, letting $\gamma$ map each
$\alpha$ to itself.
...
\sigma}) \setminus \FTV(\sigma)$.
By the induction hypothesis, there is a type substitution $\gamma$ on
domain $\FTV(\typeinterpret{\tau}) \setminus \FTV(\tau)$ such that
$\Delta \cup \{ x : $
\end{proof}
