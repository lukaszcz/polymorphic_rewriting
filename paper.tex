...
\CK{The following might need to be moved to the section where we define
interpretation terms, as these properties have nothing to do with
interpretations.\medskip}
...
\emph{infix, left-associative} notation.  Thus, $s \oplus_\sigma t
\oplus_\sigma u$ should be read as $\oplus_\sigma(s,\oplus_\sigma(t,
u))$.  $\oplus$ and $\otimes$ have the expected properties of symmetry
and associativity.
...
\Termmap(\mathtt{case}_{\alpha,\beta,\xi}) & = & \lambda x : (\alpha
  ...
  \quad & \\
  & & \multicolumn{2}{r}{
  \lift_\xi(37) \oplus_\xi (\ 
  (\lift_\xi(\flatten_{\alpha \times \beta}(x) \oplus_\nat 1))
  \otimes_\xi} \\
  & & \multicolumn{2}{r}{
  ((y \cdot \proj^1_{\alpha,\beta}(x)) \oplus_{\xi}
   (z \cdot \proj^2_{\alpha,\beta}(x)))
  \ )} \\
\end{array}
\]
...
\item $\mathtt{let}_{\qquant{\forall}{\alpha}{\sigma},\rho}(
  ...
  \item $\pi^2_{\sigma,\tau}(\mathtt{pair}_{\sigma,\tau}(s,t)) \to t$ \\
    ...
    \interpret{t}) \leadsto \interpret{t}$.
  \item $\mathtt{case}_{\sigma,\tau,\rho}(\mathtt{in}^1_{\sigma,\tau}(u),
    ...
    \lift_\rho(37) \oplus_\rho (\varphi\,\otimes_\rho (\ (\abs{x:
    ...
    and $\otimes$ and the distribution property (Lemmas
    \ref{lem:plustimesmonotonic} and
    \ref{lem:approxproperties}(\ref{lem:approx:distribution}))%
    \footnote{\CK{Actually this is not quite accurate: as the term is
    not in $\leadsto$-normal form, we cannot compare it by $\succeq$.
    What I'm trying to say here is that since we can take this step
    here, the same $\succeq$-relation will hold for its $\leadsto$-%
    normal form.  However, to make this properly formal we will have to
    define more exactly how interpretation terms that are not in
    $\leadsto$-normal form may be compared.}}, this term
    $\succeq \lift_\rho(37) \oplus_\rho (\abs{x}{\interpret{s}})
    \cdot \proj^1_{\sigma,\tau}(uu) \oplus_\rho (\abs{y}{\interpret{t}})
    \cdot \proj^2_{\sigma,\tau}(uu) \leadsto^* \lift_\rho(37)
    \oplus_\rho \interpret{s}[x:=\interpret{u}] \oplus_\rho
    \interpret{t}[y:=\interpret{u}]$.  By Lemma
    \ref{lem:plusparts} (and Lemma
    \ref{lem:approxproperties}(\ref{lem:approx:symmetry}) along with
    Lemma \ref{lem:compatibility})\footnote{\CK{Since Lemma
