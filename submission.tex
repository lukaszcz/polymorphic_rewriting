...
%  \item
  \CKchange{(b)}
    $\ell$ and $r$ have the same type under $\Gamma$;
%  \item
  \CKchange{and (c)}
    if $(\ell,r) \in \Rules$ then $(\delta(\ell),\delta(r)) \in
...
\CKchange{We can also use rules}
$(\tabs{\alpha}{s})*\tau \red s[\alpha:=\tau]$ without the extra
symbol, but to apply \CKchange{our method} it may be
%the method from
%Sections \ref{sec:World}--\ref{sec_rule_removal}, it is often more
convenient
to use the extra symbol, as
\CKchange{it creates more liberty in choosing an interpretation}.
%it will allow us to assign an
...
\begin{example}[Fold on heterogenous lists]\label{ex_fold_pafs}
  ...
  \end{array}
  \]
  and the following rules \CKchange{(which formally represents an
...
\begin{defn}
  ...
  \item\label{arrW:beta:abs} $\app{(\abs{x:\sigma}{s})}{t} \arrW
    s[\subst{x}{t}]$
  %\item\label{arrW:beta:tabs}
    \CKchange{and}
    $\tapp{(\tabs{\alpha}{s})}{\sigma}
    \arrW s[\subst{\alpha}{\sigma}]$
    \CKchange{($\beta$-reduction)}
  \item\label{arrW:plus:base}
    $\app{\app{\oplus_{\nat}}{n}}{m} \arrW (n+m)$
  %\item\label{arrW:times:base}
    \CKchange{and}
    $\app{\app{\otimes_{\nat}}{n}}{m}
    \arrW (n \cdot m)$
...
\CKchange{For the following definition}, we fix an
...
\begin{defn}\label{def:succ}
  ...
  $s\ R_{\sigma}\ t$ is defined coinductively by the following \CKchange{rules.
  \[
%  \begin{array}{cc}
...
%\end{defn}
%
\CKchange{For \emph{open} interpretation terms,
%the definition is extended in a natural way:
%
%\begin{defn}\label{def_closure}
%  A \emph{closure}~$\cl = \gamma \circ \omega$ is a
we define a \emph{closure}~$\cl = \gamma \circ \omega$ to be a}
...
setting, thanks to more sophisticated ``programming'' in \CKchange{both
type and function symbol} interpretations.
%
The reader is advised to consult e.g.~\cite[Chapter~11]{Girard1989}
for more background and explanations.  \CKchange{Here, we demonstrate the idea
...
we set \CKchange{$\Typemap(\List) = \forall \beta. (\forall \alpha.
...
\CK{TODO: shorten the following and move it somewhere.}

----------

Here, we state standard abbreviations for two inductive types
used later in this section. Type subscripts are dropped when clear
or irrelevant.
