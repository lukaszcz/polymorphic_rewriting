...
\LCchange{An} extension of the proof \LCchange{from} Example~\ref{ex:plus1} \LCchange{shows that}
...
\begin{example}\label{ex_fold_interpretation}
  ...
  \begin{array}{rcll}
    \Typemap(\List) & = & \multicolumn{2}{l}{
      \quant{\beta}{(\quant{\alpha}{\LCchange{\beta \arrtype
  ...
  \Termmap(\mathtt{A}) & = & \Lambda \alpha.\Lambda \beta.\lambda x. &
    x * \beta \\
  \Termmap(\mathtt{nil}) & = & & \LCchange{\Lambda \beta.\lambda f.} \abs{x}{x} \\
  ...
  \Termmap(\mathtt{foldl}) & = & \Lambda \beta.\lambda f. \lambda x.
    ...
    \LCchange{\beta \arrtype \alpha \arrtype \beta}}(f) \oplus \\
    & & & \phantom{ABCDEFGHIJ\ }
...
\LC{Above, the types didn't match. You're essentially doing
  ``fold-left'' encoding of a list, in contrast to the standard
  ``fold-right'' encoding (that's why your cons is different).}
