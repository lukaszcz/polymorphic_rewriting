...
\begin{abstract}
  ...
  lambda-calculus, our generalisation increases the power of the
...
for the investigated systems. First, we define the set of types.
...
We fix a variable context~$\Gamma$.
...
terms of~PAFSs. The pair $(\succeq,\succ)$ will be used as an ordering
...
\begin{defn}
  ...
  type~$\tau$ (under a fixed implicit context~$\Gamma$).
\end{defn}
...
\begin{defn}
  ...
  order~$\succ^A$ on $A$-terms such that:
  \begin{itemize}
...
\begin{proof}
  ...
  If $\sigma = \nat$ then $(\lift(n) u_1 \ldots u_k)\da = n \ge m =
  (\lift(m) u_1 \ldots u_k)\da$. Hence $\lift(n) u_1 \ldots u_k
  \succeq_\nat \lift(m) u_1 \ldots u_k$.

  If $\sigma = \tau_1\arrtype\tau_2$ then by the coinductive
  hypothesis $\lift(n) u_1 \ldots u_k q \succeq_{\tau_2} \lift(m) u_1
  \ldots u_k q$ for any $q \in \Iterms^f_{\tau_1}$. Hence $\lift(n)
  u_1 \ldots u_k \succeq_\sigma \lift(m) u_1 \ldots u_k$.

  If $\sigma = \forall(\alpha:\kappa)\tau$ then by the coinductive
  hypothesis $\lift(n) u_1 \ldots u_k \xi \succeq_{\sigma'} \lift(m)
  u_1 \ldots u_k \xi$ for any closed $\xi \in \Tc_\kappa$, where
  $\sigma' = \tau[\subst{\alpha}{\xi}]$. Hence $\lift(n) u_1 \ldots
  u_k \succeq_\sigma \lift(m) u_1 \ldots u_k$.
\end{proof}
...
to impredicativity of polymorphism direct induction on type structure
is not possible.
...
\begin{proof}
  Each point follows from one of the lemmas proven above,
...
{ \renewcommand{\thetheorem}{\ref{lem_succeq_subst}}
\begin{lemma}
