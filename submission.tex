...
list may have a different type. \LCchange{In practice, one would
...
defined using co-induction. \LCchange{For the following definitions, we fix an implict variable context~$\Gamma$.}
...
\LC{I'm unsure how to exactly phrase these definitions, because they all depend on an implict variable context~$\Gamma$ (including the definition above). I removed explicit references to~$\Gamma$ below to make things more uniform; change if you don't like it.}
...
\begin{defn}\label{def_closure}
  ...
  we define $s \succ_\sigma t$ if for every \LCchange{closure~$\cl$} we can
...
\CKchange{Towards a termination proof of Example \ref{ex_fold_pafs}, we will
adapt a standard interpretation of $\List$ and $\fold$ to System
$\Fomega$ (see, e.g.,
\cite[Chapter~11]{Girard1989} for more background and explanations).
...
we can see that this term $\succ \interpret{s}$.
