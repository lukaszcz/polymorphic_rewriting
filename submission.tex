...
\begin{abstract}
  ...
  non-polymorphic setting by allowing for encoding of inductive data
...
\begin{lemma}[Generation lemma]\label{lem:generation}
  ...
  \item $t \equiv \app{t_1}{t_2}$ and
    ...
    $\Gamma \vdash t_2 : \tau$ and $\FTV(\tau) \subseteq \Vc$.
...
\begin{proof}
  ...
  ensure $\FTV(\sigma') \subseteq \Xc$, note if $\alpha \notin \Xc$
...
\LC{This definition is the weakest one I came up with such that the
  proof of Lemma~\ref{lem_succinterpret_monotonic} can be carried out
  inductively without too much hassle.}
...
\LC{The following paragraph should be rewritten, because it's
  inaccurate. The whole point why we're using concrete terms in the
  interpretations is that there is no way to interpret type
  abstraction ($\Lambda \alpha$) ``set-theoretically''. It's therefore
  not true that our interpretation terms correspond to ``functionals''
  in the ordinary sense. They do if we disregard type abstractions.}
