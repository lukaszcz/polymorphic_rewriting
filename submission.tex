...
implicit context~$\Gamma$, or equivalently with~$t$ considered an
...
\begin{example}[Fold on heterogenous lists]\label{ex_fold_pafs}
  ...
  programming language or in \CKchange{the polymorphic higher-order
  rewriting formalisms of \cite{jou:oka:91,jou:rub:99}}. The above rules
  cannot be translated into an
  infinite set of simply typed rules by instantiating the type
  variables. In practice, when defining heterogenous lists in a
  language with support for higher-rank polymorphism (e.g.~in recent
  extensions of Haskell, or in a type-theory based proof assistant
  like~Coq), one would constrain the type variable~$\alpha$ above with
  a type class to guarantee the existence of certain operations on the
...
For easier presentation, we will use $\oplus$ and $\otimes$ in
...
\begin{proof}
  ...
  hypothesis. Thus $t_1\succeq_{\forall\alpha\sigma} t_3$.
\end{proof}
...
\begin{lemma}\label{lem_succeq_subst}
  If $s \succeq_\sigma s'$ then $t[\subst{x}{s}] \succeq_\tau
  t[\subst{x}{s'}]$.
\end{lemma}
...
\begin{defn}
  ...
  A fixed type constructor mapping $\Typemap$ is extended inductively
  to a function from type constructors to closed interpretation type
  constructors in the expected way.

  We denote the extended mapping by~$\typeinterpret{\sigma}$. Thus,
  for example $\typeinterpret{\quant{\alpha}{\sigma}} =
...
\begin{defn}
  ...
  \[
    ...
    \end{array}
  \]
  Symbol mapping is extended to contexts: \( \itp{\Gamma} := \{ (x :
  \typeinterpret{\tau}) \mid (x : \tau) \in \Gamma \}.  \)
\end{defn}
...
The above theorem gives rise to the following algorithm:
\begin{enumerate}
...
\subsection{Calculation rules}\label{sec_calculation_rules}

We state a few lemmas which are helpful in showing rule orientation
with~$\succeqinterpret$ and~$\succinterpret$. The proofs may be found
in Appendix~\ref{app_proofs}.
...
\subsection{Proofs for Section~\ref{sec_calculation_rules}}
