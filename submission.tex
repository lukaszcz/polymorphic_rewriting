...
\begin{abstract}
  ...
  make essential use of impredicative polymorphism.  In addition,
  our generalisation eases the applicability of the method
...
\begin{defn}\label{def_types}
  ...
  % CK: space, and this fits together well enough, I think.
...
\pagebreak
  ...
  in~$t$.
  %We say that $t$ is \emph{closed} if $\FV(t) = \emptyset$ and
  %$\FTV(t) = \emptyset$.
...
%\emph{closed} if its canonical representative is.
We define $\FTV(t)$
as the value of~$\FTV$ on the canonical representative of~$t$.
%CK: moved this to save a line, which allows 2.1 to start at the top of
%page 3.
\CKchange{We say that $t$ is \emph{closed} if both $\FTV(t) = \emptyset$
and $\FV(t) = \emptyset$.}
%
...
\begin{lemma}[Generation lemma]\label{lem:generation}
  ...
  =_\beta \sigma$ and $\FV(\sigma') \subseteq \Xc$ and one of the
  following holds.
  \begin{itemize}
...
\begin{proof}
  ...
  ensure $\FV(\sigma') \subseteq \Xc$, note if $\alpha \notin \Xc$
  is of kind~$\kappa$ and~$\Gamma' \proves s : \sigma'$
  with~$\FTV(s) \cup \FTV(\Gamma') \subseteq \Xc$, then $\Gamma
  \proves s : \sigma'[\subst{\alpha}{\chi_\kappa}]$ by the
  substitution lemma (thus we can eliminate~$\alpha$).
\CK{I do not understand this proof sketch. 
$\Vc$ is defined as a set of
...
\CK{But in Definition 2.3, $\Vc$ refers to the set of variables, not
  type constructor variables.  I would rather use a different letter
  here -- so I have now replaced it by $\Xc$.  However, I still do not
  understand what is happening with the use of $\chi_\kappa$.  Where
  does $\Gamma'$ come from?}
\CK{Separately: I \emph{do} think it may be better to just consistently
  use $\FTV$ for free type variables (also in types) and $\FV$ for term
  variables.}
\end{proof}
...
\begin{defn}[Heterogenous lists]
  %CK: Here space saving will not work, as the cons is too large.
  \[
  \begin{array}{rcl}
...
\CKchange{From the definition we immediately see that all encodings have the expected type:}
...
\CKchange{In addition, $\beta$-reduction on terms and types -- that is, the
relation $\leadsto_\beta$ generated by $(\abs{x}{s})t \leadsto_\beta s[x:=t]$
and $(\tabs{\alpha}{s})\sigma \leadsto_\beta s[\alpha:=\sigma]$ -- provides
the desired derivation rules:}
...
\CKchange{In this section, we will present a form of higher-order term rewriting
systems based on $\Fomega$: \emph{Polymorphic Functional Systems (PFSs)}.
Systems of interest, such as logic systems like ICP2 and higher-order
TRSs with shallow or full polymorphism can be encoded into PFSs, and then proved
terminating with the technique we will develop in Sections
\ref{sec:World}--\ref{sec_rule_removal}.}
...
\begin{defn}\label{def_pafs_types_terms}
  ...
  %CK: space restructuring
  \CKchange{We assume given a fixed set~$\Sigma$ of function symbols, such
  ...
  Definition~\ref{def_preterms}) with the \CKchange{restriction that
...
\CK{Note: this definition \emph{does} allow us to have a subterm
$\mathtt{f}_{\rho_1,\ldots,\rho_n}(s_1,\ldots,s_m)$ with $m < k$ that
does not occur at the head of an application.  I do not mind this at
all.  However, it does mean I had to change the text below a little
to allow for it, so if you feel that this should not be allowed,
change the part with $m \leq k$.  The definition also allows us to
have $s * \tau$ directly rather than encoded.  Are you happy to have
this?  I am fine with it, since I do not believe that it will be in the
way of monotonicity, but I also do not need it.  Again, I did adapt the
text to take into account that it is allowed.}

\CKchange{Note that this definition does not allow for a variable or
...
$
  ...
  \sigma_k \arrtype \tau
$
is} a function symbol in~$\Sigma$ with~$\tau$ a type atom \CKchange{and $m \leq k$.
%CK: omitting this, because it is implied by
%  $\mathtt{f} \rho_1 \ldots \rho_n s_1 \ldots s_k$ being a term in the
%  first place.
%and $\rho_i$
%is a type constructor of kind $\kappa_i$ for $i=1,\ldots,k$, and
%$\Gamma \proves s_i : \sigma_i[\alpha_1 := \rho_1]\ldots[\alpha_n :=
%  \rho_n]$ for $i=1,\ldots,k$, for an appropriate~$\Gamma$.
...
\CKchange{The following result follows easily by induction on term
structure:}
...
\CKchange{PFS terms will be rewritten through a reduction relation
$\arr{\Rules}$ based on a (usually infinite) set of rewrite rules.
To define this relation, we need two additional notions.}

\begin{defn}\label{def_replacement}
  \CKchange{For a variable context $\Gamma$,
  ...
  \delta(\tau)$.
  \CK{The set $\Iterms$ is not defined here; can you reformulate this?}
\end{defn}
...
\CKchange{Now, the} rewrite rules are simply a set of term pairs, whose
...
\begin{defn}\label{def_pafs}
  ...
  of rules as in Definition~\ref{def_rules}. A term of \CKchange{a %given
  PFS~$A$} is referred to as an $A$-term.
\end{defn}

\CKchange{While PFS-terms are a restriction from the general terms
...
\begin{example}[Fold on heterogenous lists]\label{ex_fold_pafs}
  ...
  \begin{array}{rcl}
    ...
    \CKchange{\mathtt{A}_{\abs{\alpha}{\sigma},\tau}}(\tabs{\alpha}{s}) & \red &
    s[\alpha:=\tau] \\
  ...
  each \CKchange{choice of types $\sigma,\tau$ and PFS terms $s$, $t$}, etc.
\end{example}

\section{A well-ordered set of interpretation terms}\label{sec:World}

\CKchange{In polynomial interpretations of first-order term
...
in a specific extension of $\Fomega$.  This set is defined in Section
\ref{subsec:I}; we provide a well-founded partial ordering $\succ$ on
$\Iterms$ in Section \ref{subsec:succ}.}

\CKchange{By construction, our interpretation terms define weakly
monotonic functionals. Our method does not allow for arbitrary functions
like the weakly monotonic algebras of \cite{pol:96}; however, it does
strictly extend the \emph{polynomial interpretations} of
\cite{FuhsKop2012}.}
...
%CK: delegated to the appendix, since we do not use \SN at all other
%than in the theorem below, which can be reformulated.
...
\begin{proof}
  ...
  type~$\tau'$ is smaller than~$\tau$. This %is impossible by
  \CKchange{contradicts} %CK: space
  the
...
of~PFS function symbols and a reduction pair
$(\succeqinterpret,\succinterpret)$ on PFS terms. In the next
