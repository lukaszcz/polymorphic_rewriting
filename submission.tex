...
\begin{abstract}
  ...
  %CK: reformulating this because it's a long and rather hard-to-read
  %sentence, and because I wanted to save a line.
  \CKchange{This enables} a direct interpretation of rewrite rules which
  make essential use of impredicative polymorphism\CKchange{.  Our
  generalisation also} eases the applicability of the \CKchange{method}
  in the non-polymorphic setting \CKchange{by allowing for an encoding
  of} inductive data \CKchange{types.} %in the polymorphic lambda-calculus.
...
%CK: _polynomial_ interpretations are not complete for first-order
%rewriting! Algebra interpretations are. And Jaco never introduced
%higher-order polynomials; that definition is mine.
\CKchange{One powerful method, introduced by v.d. Pol \cite{Pol1993,pol:96},
...
heterogenous lists\CKchange{.
%CK: creating space; you may undo this if you think it is not the right
%place to cut space
%\[
%\begin{array}{l}
%  \List : * \\
...
\end{array}
\]
T}he above states that $\List$ is a type ($*$), \CKchange{gives} the types of its
...
\CK{I would be inclined to remove this paragraph from the introduction,
as it does little to motivate our work or the problem we solve, but
rather causes a distraction.}
...
%CK: This comes across a little week, I think. Reformulating!
...
%in System~$\mathtt{F}$.
%In Section~\ref{sec:examples}, we study a more complex example --
\CK{The technique we will develop in this paper can be used to handle
systems like the one above.  However, termination of heterogeneous fold
can be shown in other ways (e.g., an encoding in System~$\mathtt{F}$).
Hence, we will also study a more complex example in
Section~\ref{sec:examples}:}
termination of a substantial fragment of~IPC2,
...
methodology.
\CKchange{It is our goal to make termination proofs for systems like
this substantially easier in the future.}

\CK{I would be inclined to move this part to the example section; I
think it would fit better in a discussion of what we have and what we
do not get.  It would be a good fit here if we actually did solve the
open problem, but at present we don't.}
Pol and Schwichtenberg used higher-order polynomial
...
\CKchange{We assume familiarity with core notions of lambda
...
\begin{defn}\label{def_types}
  ...
  \CK{This example is not part of the running example (which is very boring
  in the way of types), but I figured something like this should be included
  because type constructors having a higher kind was very unfamiliar to me
  at first, and we may get RTA reviewers.}
  \CKchange{\begin{example}
  ...
  *} = \{ \Pair \}$, types are for instance $\List$ and
  ...
  constructors. \CKchange{As} type constructors are \CKchange{(mostly)} simply-typed
  lambda-terms, \CKchange{their $\beta$-reduction} %on type constructors
  terminates
  and is confluent\CKchange{; hence} every type constructor~$\tau$ has a unique
  $\beta$-normal form~$\nf_\beta(\tau)$. A \emph{type atom} is a type
  in $\beta$-normal form which is not an arrow $\tau_1\arrtype\tau_2$
  or \CKchange{quantification} $\forall\alpha.\tau$.
...
\begin{defn}\label{def_preterms}
  ...
  \item $\Gamma \vdash \tabs{\alpha}{s} : \quant{\alpha}{\sigma}$ if
    ...
    \CK{Given the conventions you set earlier, it is implied here that $\alpha$
    is a type variable, which I don't think is intended.  Should we change this
    to $\tabs{\alpha:\kappa}{s} : \quant{\alpha:\kappa}{\sigma}$ with a note
    that if no kind is specified, it should be assumed to be *?}
...
\begin{lemma}[Generation lemma]
  ...
  \item $t \equiv \tabs{\alpha}{s}$ and $\sigma' =
    ...
    \notin \FTV(\Gamma)$.
    \CK{Same question applies as before: should there be kinds added?}
...
%CK: shortening, but easily undone if you disagree with the aesthetics
%  \[
%  \begin{array}{rcl}
...
\CK{We don't have $\succ$ yet, so we cannot do this here.  It is part of the lemma on safety regardless, though.  Remove?}
\begin{lemma}
  If $t \succ t'$ then $\pi^i(t) \succ \pi^i(t')$.
\end{lemma}

\begin{defn}[Existential types]
  %CK: Space, but easily undone
%  \[
%  \begin{array}{rcl}
...
\CK{Same here: $\succ$ does not yet exist.}
\begin{lemma}
