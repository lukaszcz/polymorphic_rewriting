...
\CK{Is it accurate this way? (Please feel free to reformulate!)}

\CKchange{We use concrete terms rather than functionals since type
abstraction ($\Lambda \alpha$) cannot be interpreted in a set-theoretic
way.  However, if type abstraction is disregarded, our interpretation
terms define weakly monotonic functionals by construction.
Our method does not allow for arbitrary functions like the weakly
monotonic algebras of \cite{pol:96}, but does extend
the \emph{polynomial interpretations} of \cite{FuhsKop2012}.}
