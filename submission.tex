...
\begin{defn}\label{def_types}
  ...
  \kappa_2$. We assume infinitely many \emph{type constructor
    variables} of each kind. Variables of kind~$*$ are \emph{type
    variables}. We assume a fixed set~$\Sigma_T$ of \emph{type
    constructor symbols} paired with a kind, denoted $c :
  \kappa$. Every type constructor symbol~$c$ occurs with only one kind
  declaration. We assume there exists a fixed type symbol~$\chi_*$ (of
  kind~$*$). For $\kappa=\kappa_1\arrkind\kappa_2$ we define
...
\begin{defn}\label{def_preterms}
  ...
  $\Gamma$ refer to a set of pairs $x : \tau$ of a variable and a
  type, such that each variable occurs at most once. We assume given a
  fixed set $\Sigma$ of \emph{function symbols}, each paired with a
  closed type, denoted $\mathtt{f} : \tau$.  Every function symbol
  $\mathtt{f}$ occurs only with one type declaration. We use
  $\Gamma,x:\tau$ for $\Gamma \cup \{x:\tau\}$. We set $\FTV(\Gamma) =
  \bigcup\{\FV(\tau) \mid (x : \tau) \in \Gamma\}$.
...
setting, through more sophisticated ``programming'' in the
interpretations.
...
analyse, we define the framework of Polymoprhic Algebraic Functional
...
\begin{defn}\label{def_rules}
  ...
  s$ iff $t = C[\delta(\ell)]$ and $s = C[\delta(r)]$ for some
  $(\ell,r)\in\Rules$, a replacement~$\delta$, and a context~$C$.
\end{defn}

\begin{defn}\label{def_pafs}
  A \emph{Polymorphic Algebraic Functional System (PAFS)} is a triple
  $(\Sigma_T,\Sigma,\Rules)$ where~$\Sigma_T$ is a set of type
...
\begin{example}[Fold on heterogenous lists]\label{ex_fold_pafs}
  ...
  programming language or in the polymorphic higher-order rewriting
  formalisms of~\cite{jou:oka:91,jou:rub:99}. The above rules cannot
  be translated into an infinite set of simply typed rules by
  instantiating the type variables. In practice, when defining
  heterogenous lists in a language with support for higher-rank
  polymorphism (e.g.~in recent extensions of Haskell, or in a
  type-theory based proof assistant like~Coq), one would constrain the
  type variable~$\alpha$ above with a type class to guarantee the
  existence of certain operations on the elements of the list.
\end{example}
...
For easier presentation, we write $\oplus_\tau$, etc., instead of
$\tapp{\oplus}{\tau}$, etc. We will also use $\oplus$ and $\otimes$ in
...
\begin{defn}
  ...
  the same type~$\sigma$). A binary relation~$R$ on $A$-terms is
  \emph{stable} if $R(s, t)$ implies $R(\delta(s), \delta(t))$ for
  every replacement~$\delta$.
  ...
  \item $\succeq^A$ and~$\succ^A$ are both stable,
...
\begin{proof}
  ...
  Lemma~\ref{lem:compatibility}. Stability follows from definitions,
  by manipulating closures. Monotonicity of~$\succeqinterpret$ follows
  from Lemma~\ref{lem_succeq_subst}. Monotonicity of~$\succinterpret$
  follows from Lemma~\ref{lem_succinterpret_monotonic}.
\end{proof}
...
\begin{proof}
  Monotonicity and stability of~$\succeq^\Rules$ and~$\succ^\Rules$
  imply that ${\arr{\Rules_1}}\subseteq{\succ^\Rules}$ and
