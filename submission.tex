...
is no test for zero).
%SPACE
%For the purposes of termination, the type~$\nat$
...
%  \forall\alpha\sigma$.
  \CKchange{If $\tau \in \ITypes$ is closed and $\beta$-normal, then
...
\begin{proof}
  ...
  for any closed %type constructor~
  $\tau$ of kind~$\kappa$, where
  $\sigma' = \nf_\beta(\sigma[\subst{\alpha}{\tau}])$. \CKchange{By the
  coinductive hypothesis}
  $\tapp{t_1}{\tau}\succ_{\sigma'}\tapp{t_3}{\tau}$\CKchange{; thus}
...
\begin{lemma}\label{lem_succ_red}
  ...
  %\begin{enumerate}
  %\item
  \CKchange{If $t \leadsto t'$ or $t' \leadsto t$ then $t' \succ s$
    (resp.~$t' \succeq s$).
  %\item
  If $s \leadsto s'$ or $s' \leadsto s$ then $t \succ s'$
    (resp.~$t \succeq s'$).}
  %\end{enumerate}
\end{lemma}
...
\begin{lemma}\label{lem_app_succ}
  \CKchange{For $R \in \{\succeq,\succ\}$:
  if $t\:R\:s$ then $t u\:R\:s u$ with $u$ a term or type constructor.}
\end{lemma}
...
\begin{lemma}\label{lem:liftgreater}
  ...
  %all types $\sigma$.
  If $n\:R\:m$ then $\lift_\sigma n\:R\:\lift_\sigma m$ for all types
  $\sigma$.}
\end{lemma}
...
\begin{proof}
  ...
  \CKchange{for} conciseness). If $\sigma = \nat$ then $(\lift(n) u_1 \ldots
...
\begin{lemma}\label{lem_flatten_succ}
  \CKchange{For $R \in \{\succeq,\succ\}$:
  f $t\:R_\sigma\:s$ then $\flatten_\sigma t\:R_\nat\:
...
\begin{proof}
  ...
  $\beta$-normal form.
  Using Lemma~\ref{lem_succ_red}, %one shows by
  ...
  \CKchange{the lemma follows by induction on~$\sigma$.}
\end{proof}

\begin{lemma}\label{lem_abs_succ}
  \CKchange{For $R \in \{\succeq,\succ\}$:
  if $t\:R\:s$ then $\abs{x}{t}\:R\:\abs{x}{s}$ and
...
\begin{proof}
  ...
  that for closed $w_1,\ldots,w_n$ \CKchange{(denoted $\vec{w}$):
  if $s \vec{w} \succ t \vec{w}$ and $u \vec{w} \succeq \lift(1)
  \vec{w}$
...
\begin{defn}
  ...
  maps \CKchange{each} type constructor symbol to a closed interpretation type
...
Similarly, we employ a \emph{symbol mapping} as \CKchange{the key
...
\begin{lemma}\label{lem:substitutioninterpret}
  \CKchange{For all $s,t,x,\alpha,\tau$:
  %\begin{enumerate}
...
% LC: I don't think we have any "programming" to speak of in the types
% (and I don't think we need it).
The reader is advised to consult e.g.~\cite[Chapter~11]{Girard1989}
for more background and explanations. \CKchange{We} demonstrate the idea
by presenting an encoding for the recursive type $\List$ and \CKchange{its}
%corresponding
fold-left function \CKchange{(see also
...
\begin{lemma}\label{lem_safe}
  ...
  \item If $t$ is safe for~$x$ then so \CKchange{are}~$\lift(t)$
    and~$\flatten(t)$.
...
\begin{proof}
  ...
  instance, \CKchange{for the first,} assume $s_1 \succ s_2$ and let
...
\begin{proof}
  Assume $s_1 \succinterpret s_2$. By induction on %the structure of
  \CKchange{a}
  ...
  %and thus
  \CKchange{so} $C[s_1] \succinterpret C[s_2]$ follows from definitions.
...
\begin{theorem}\label{thm:ruleremove}
  ...
  \CKchange{(so certainly if $\Rules_2 = \emptyset$).}
...
(Lem.~\ref{lem:substitutioninterpret}).  \CKchange{We will also use
Lemma \ref{lem:plustimesmonotonic} which states (among other things)
that $s \succ t$ implies that $s \oplus u \succ t \oplus u$.}
...
\CKchange{Lemmas~\ref{lem:plustimesmonotonic} and}~\ref{lem:plusparts} we
...
t_1$ then $s \succ t$.  This property \CKchange{is typically very
...
\CKchange{In this section we show}
...
\begin{remark}
\CKchange{Our method builds on the work of Pol and Schwichtenberg,}
who have also used higher-order polynomial interpretations to prove
termination of a fragment of intuitionistic first-order logic with
permutative conversions~\cite{PolSchwichtenberg1995}, in the hope of
providing a more perspicuous proof of this well-known result. Notably,
they did not treat disjunction\CKchange{, as we will do}. Also, their method
...
And 28 reduction rules, including the following (the full set of rules is in \CKchange{Appendix \ref{app_ineqs}}):
\[
\begin{array}{rclrcl}
@_{\sigma,\tau}(\abs{x}{s},t) & \red & s[x:=t] &
...
\[
\begin{array}{l}
\CKchange{@_{\sigma,\tau}(\epsilon_{\sigma \arrtype \tau}(s),t) \red
  \epsilon_\tau(s)} \\
\CKchange{\mathtt{case}_{\sigma,\tau,\rho}(\epsilon_{\mathtt{or}\,\sigma\,\tau}(
  u),\abs{x}{s},\abs{y}{t}) \red \epsilon_\rho(u)} \\
...
To define an intepretation for~IPC2, we \CKchange{will use the}
...
\CKchange{The derivations to orient rules with these
\pagebreak
interpretations are also given in Appendix \ref{app_ineqs}.}
...
for non-polymorphic (\LC{Cynthia: shallowly polymorphic?}\CK{No.})
higher-order rewriting~\cite{FuhsKop2012}.
...
\CKchange{As the method is not that easy to apply manually for larger
systems,}
...
%the proofs is tedious.  Hence,
a natural direction for future work is
...
other techniques for higher-order termination such as path orderings
(see, e.g., \cite{jou:rub:99}) or dependency pairs (e.g.,
...
\LC{TODO: The formatting (spacing) gets pretty screwed up below.}
