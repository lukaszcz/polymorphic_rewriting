...
%STORY: We can try to use the same idea for higher-order rewriting, as seen for instance in functional programming and various proof systems.
...
\ \\\pause
\textbf{Answer:} Depends on the type. \pause It terminates if:
\[
\begin{array}{rcl}
\nil & : & \List \\
...
\begin{frame}{Bonus slide because people might just ask}

%STORY: In slide 2 I mention that termination of map actually depends on the type. Not everyone will believe it, so if they ask, here is the example of non-termination.
...
\textbf{Answer:} Depends.
%NOTE: The following is optional. I would be inclined to show it, but go over it very quickly, to show that types are important, but not waste too much time on it.
