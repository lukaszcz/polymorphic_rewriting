...
\end{array}
\]
% STORY: List : * => * is a function from a type to a type (so then List(alpha) is a type)
\[
\begin{array}{rclr}
...
\end{array}
\]
% STORY: Now fold, cons, nil also take types as arguments; e.g. nil :
% forall alpha . List(alpha) means nil takes a type argument alpha and
% returns an element of type List(alpha) -- recall List is a function
% from types (*) to types (*), so List(alpha) is a type
...
\end{array}
\]
% STORY: Here the list may contain elements of arbitrary types; the
% function F receives the type of the element together with the
% element.
...
% STORY: About tapp: we do here an analogous thing to term
% application, but for type application, don't worry about the details
% of tapp right now.
