...
less perspicuous and more difficult to formalise.
...
\begin{proof}
  It suffices to show this for~$\succ^0$ and closed terms and closed types in
  $\beta$-normal form, because any infinite sequence $t_1 \succ_\tau^0
  t_2 \succ_\tau t_3 \succ_\tau \ldots$ induces an infinite sequence
  $\cl(t_1) \succ_{\nf_\beta(\cl(\tau))}^0 \cl(t_2)
  \succ_{\nf_\beta(\cl(\tau))}^0 \cl(t_3) \succ_{\nf_\beta(\cl(\tau))}^0
  ...
  not exist an infinite sequence $t_1 \succ_\tau^0 t_2 \succ_\tau^0 t_3
  \succ_\tau^0 \ldots$ For instance, if $\alpha$ has kind~$\kappa$ and
  $t_1 \succ_{\forall\alpha\tau}^0 t_2 \succ_{\forall\alpha\tau}^0 t_3
  \succ_{\forall\alpha\tau}^0 \ldots$ then $\tapp{t_1}{\chi_\kappa}
  \succ_{\tau'}^0 \tapp{t_2}{\chi_\kappa} \succ_{\tau'}^0
  \tapp{t_3}{\chi_\kappa} \succ_{\tau'}^0 \ldots$, where
...
\begin{proof}
  ...
  analogous. Again, it suffices to prove this for~$\succ^0$ and closed
  terms and closed types in $\beta$-normal form. We proceed by
  coinduction.

  If $t_1 \succ_\nat^0 t_2 \succ_\nat^0 t_3$ then $t_1\da > t_2\da >
  t_3\da$, so $t_1\da > t_3\da$. Thus $t_1 \succ_\nat^0 t_3$.

  If $t_1 \succ_{\sigma\arrtype\tau}^0t_2\succ_{\sigma\arrtype\tau}^0t_3$
  then $\app{t_1}{q}\succ_{\tau}^0\app{t_2}{q}\succ_\tau^0\app{t_3}{q}$
  for $q \in \Iterms^f_\sigma$. Hence
  $\app{t_1}{q}\succ_\tau^0\app{t_3}{q}$ for $q \in \Iterms^f_\sigma$ by
  the coinductive hypothesis. Thus $t_1\succ_{\sigma\arrtype\tau}^0
  t_3$.

  If $t_1
  \succ_{\forall(\alpha:\kappa)\sigma}^0t_2\succ_{\forall(\alpha:\kappa)\sigma}^0t_3$
  then
  $\tapp{t_1}{\tau}\succ_{\sigma'}^0\tapp{t_2}{\tau}\succ_{\sigma'}^0\tapp{t_3}{\tau}$
  for any closed~$\tau$ of kind~$\kappa$, where
  $\sigma' = \nf_\beta(\sigma[\subst{\alpha}{\tau}])$. By the
  coinductive hypothesis
  $\tapp{t_1}{\tau}\succ_{\sigma'}^0\tapp{t_3}{\tau}$; thus
  $t_1\succ_{\forall\alpha\sigma}^0t_3$.
\end{proof}
...
\begin{proof}
  By coinduction one shows that $\succeq_\sigma^0$ is reflexive on
...
\begin{proof}
  ...
  $\lift(n) u_1 \ldots u_k \succeq^0 \lift(m) u_1 \ldots u_k$ for closed
  ...
  hypothesis
  $\lift(n) u_1 \ldots u_k q \succeq_{\tau_2}^0 \lift(m) u_1 \ldots u_k
  q$ for any $q \in \Iterms^f_{\tau_1}$, so
  $\lift(n) u_1 \ldots u_k \succeq_{\sigma}^0 \lift(m) u_1 \ldots u_k$
  by definition. If $\sigma = \forall(\alpha:\kappa)\tau$ then by the
  coinductive hypothesis
  $\lift(n) u_1 \ldots u_k \xi \succeq_{\sigma'}^0 \lift(m) u_1 \ldots
...
\begin{proof}
  ...
  $s \vec{w} \succ^0 t \vec{w}$ and $u \vec{w} \succeq^0 \lift(1) \vec{w}$
  then $(s \otimes u) \vec{w} \succ^0 (t \otimes u) \vec{w}$.
\end{proof}
...
\begin{remark}
Our method builds on the work of Pol and Schwichtenberg, who used
...
\LC{Can we put this sentence somewhere else? It feels a bit out of
  place for me, because here we're talking about how Pol \&
...
The derivations to orient rules with these
...
step toward a systematic, extensible methodology of termination proofs
