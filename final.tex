...
\CKchange{We will use the system of heterogeneous fold above as a running
...
\CKchange{Complete proofs for the results in this paper are available
...
\CKchange{$\mathtt{A}_{\abs{\gamma}{\sigma},\tau}(\tabs{\gamma}{s}) \red s[\gamma:=\tau]$.}%
  \footnote{\CKchange{The use of a type constructor variable $\alpha$ of kind
...
\CKchange{making it possible} to compare two arbitrary types. Our rules do not allow such a
...
normalisation. Because of space limitations, most \CKchange{(complete)}
proofs are delegated to \CKchange{\cite[Appendix~A.1]{versionwithappendix}.}
