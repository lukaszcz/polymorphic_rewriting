...
\begin{defn}\label{def_types}
  ...
  *} = \{ \Pair \}$, types are for instance $\List$ and
  ...
  in $\beta$-normal form which is \CKchange{neither} an arrow $\tau_1\arrtype\tau_2$
  \CKchange{nor a} quantification $\forall\alpha.\tau$.
  ...
  \CKchange{\Sigma^T_*}$. For $\kappa=\kappa_1\arrkind\kappa_2$ we define
...
$\CKchange{\oplus_\sigma\,(\oplus_\sigma\,s\,t)\,u}$ if $s$ has type $\sigma$.
...
this holds if $s u \oplus 1 \succ \CKchange{s} u$.  By definition, this is the
case if $(s u \oplus 1)\downarrow > \CKchange{(s u)}\downarrow$ in the natural numbers,
...
for $x$, since $x$ is not an argument of $\mathtt{cons}$\CKchange{:
...
heterogeneous lists \CKchange{(still using the interpretation from
...
%
For the last rule note that \CKchange{(using only Lemmas
...
\CKchange{Again, the types represent formulas in intuitionistic
second-order propositional logic, and the terms represent proofs.
For example, a term $\mathtt{case}_{\sigma,\tau,\rho}\ s\ u\ v$
is a proof term of the formula $\rho$, built from a proof $s$ of
$\mathtt{or}\ \sigma\ \tau$, a proof $u$ that $\sigma$ implies $\rho$
and a proof $v$ that $\tau$ implies $\rho$.
Proof terms can be simplified using 28 reduction rules, including
the following} (the full set of rules is available in Appendix
\ref{app_ineqs}):
\[
\begin{array}{rclrcl}
@_{\sigma,\tau}(\abs{x}{s},t) & \red & s[x:=t] &
...
\CKchange{\proj^2_{\rho,\pi}(\mathtt{case}_{\sigma,\tau,\mathtt{and}\,\rho,\pi}(u,
...
\CKchange{We do not currently have a structural method to find a
