...
\CKchange{The methods of weakly monotonic algebras and polynomial
interpretation are both limited to \emph{monomorphic} systems.}
In this paper, we \CKchange{will further generalise polynomial}
...
%polymorphism.}
%
\CKchange{This goes beyond shallow (rank-1, weak) polymorphism,
where type quantifiers are effectively allowed only at the top of a
type: it would be relatively easy to extend the methods to such a
system since shallowly polymorphic rules can be seen as defining an
...
forms of polymorphism.  To start, there are several lines of research
that consider shallow polymorphic rewriting
...
includes full impredicative polymorphism.  The termination methods
for these systems are mostly syntactic methods, such as the recursive
path ordering \cite{jou:rub:07,wal:03}, or general schema
\cite{bla:05}.  An exception is \cite{dow:17}, which defines
interpretations into $\Pi$-algebras; this technique bears some
similarity to the one we will define, although the methodologies are
...
termination methods are considered for this framework.
...
systems like the one above.
\CKchange{Due to the additional complications of full polymorphism, we have
elected to only generalise higher-order polynomial interpretations,
and not v.d. Pol's weakly monotonic algebras; e.g., we fix the natural
numbers as the base algebra and only consider addition and
multiplication as operations.  We believe that the same ideas could be
used to obtain a true generalisation of v.d. Pol's work, but it would
be harder to present concisely.}

\LC{I'm not sure if I believe that we can obtain a true generalisation
of v.d. Pol's work, because our approach is (and needs to be) more syntactic.}
...
Complete proofs for the results in this paper are available
in an online appendix.~\cite{versionwithappendix}.
...
for the investigated systems. \LCchange{In comparison to System~$\mathrm{F}$,
...
\LCchange{Note that we present terms in orthodox Church-style, i.e.,
...
\begin{defn}\label{def_rules}
  ...
  \CKchange{on PFS terms} is defined by:
...
$\sigma$. Thus, our interpretation terms include natural
  ...
  not cause any fundamental problems to add more \CKchange{monotonic} operations, e.g.,
...
\begin{defn}
  ...
  equivalence classes of~$\equiv$. So\CKchange{, for instance,
  $\lift_\nat$ above} denotes
...
\begin{proof}
  ...
  \CKchange{Details are available in \cite[Appendix A.1]{versionwithappendix}.}
\end{proof}
...
proofs less perspicuous. \LC{Moreover, a coinductive definition is better
...
found in \CKchange{\cite[Appendix~A.2]{versionwithappendix}}. The proof is actually quite
...
\CKchange{\cite[Appendix~A.3]{versionwithappendix}}.
...
set of rules is available in \CKchange{\cite[Appendix~B]{versionwithappendix}}:
\[
\begin{array}{rclrcl}
@_{\sigma,\tau}(\abs{x}{s},t) & \red & s[x:=t] &
...
\CKchange{\cite[Appendix~B]{versionwithappendix}}.
