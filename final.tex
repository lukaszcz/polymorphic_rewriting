...
\newcommand{\onlyarxiv}[1]{\textcolor{red}{#1}}
...
\medskip\noindent\CKchange{\textbf{Related work.} The term rewriting literature
...
\medskip\noindent\CKchange{\textbf{Our approach.}
...
with full impredicative formalism (Section \ref{sec_systems}), that
...
interpreted to combinations of addition and multiplication.  It may be
possible to extend the same ideas to obtain a true generalisation of
v.d. Pol's work, but it is not immediately obvious how because the
polynomial shape of interpretations is inherent in our core set
$\Iterms$.}

%The technique we develop in this paper can be used to handle
%systems like $\mathtt{foldl}$ above.
...
\onlypaper{Complete proofs for the results in this paper are available
...
\begin{defn}\label{def:context}
  A \emph{$\sigma$-context}~$C_\sigma$ is a \CKchange{PFS} term with a fresh function
  symbol $\Box_\sigma \notin \Sigma$ of type~$\sigma$ occurring
  exactly once. By~$C_\sigma[t]$ we denote a \CKchange{PFS} term obtained
...
Recall that our goal is to prove termination of \CKchange{reduction in
a PFS}.  To
